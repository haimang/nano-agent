...
/**
 * P2 Phase 5 — root e2e #1: tool.call live loop (in-process harness).
 *
 ...
 * preview URL dependency) and asserts the three load-bearing contracts:
 *   (a) CAPABILITY_WORKER binding is declared in wrangler.jsonc (D07);
 *   (b) composition routes capability transport to service-binding
 *       when CAPABILITY_WORKER is present (Q2a default);
 *   (c) a mock CAPABILITY_WORKER.fetch is reachable via the transport
 *       seam — same contract that live `nano-agent-bash-core-preview`
 *       exposes via `/capability/call` + `/capability/cancel`.
 *
...
test("(b) composition factory routes capability to service-binding when CAPABILITY_WORKER bound (Q2a default)", async () => {
  ...
  const handles = factory.create(
    { SESSION_DO: {}, CAPABILITY_WORKER: fakeBinding },
...
test("(c) capability transport seam invokes CAPABILITY_WORKER.fetch end-to-end (mock binding)", async () => {
  const { createDefaultCompositionFactory } = await import(
    "../workers/agent-core/dist/host/composition.js"
  );
  const { DEFAULT_RUNTIME_CONFIG } = await import(
...
test("(d) R2 wire-truth guard: agent-core source references session.start.initial_input and session.followup_input.text but NOT a 'turn_input' wire kind", async () => {

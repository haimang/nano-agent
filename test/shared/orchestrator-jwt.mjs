...
export async function signOrchestratorJwt(payload, secret, expiresIn = 3600) {
  const now = Math.floor(Date.now() / 1000);
  const headerB64 = base64Url.encode(new TextEncoder().encode(JSON.stringify({ alg: "HS256", typ: "JWT" })));

...
async function dispatchFetch(request: Request, env: OrchestratorCoreEnv): Promise<Response> {
    ...
    if (modelDetailRoute) {
      return handleModelDetail(request, env, modelDetailRoute);
    }
    ...
    if (confirmationRoute) {
      return handleSessionConfirmation(request, env, confirmationRoute);
    }
...
function parseSessionConfirmationRoute(
  request: Request,
): SessionConfirmationRoute | null {
  const pathname = new URL(request.url).pathname;
  const method = request.method.toUpperCase();
  ...
  const detailOrDecision = pathname.match(
    /^\/sessions\/([^/]+)\/confirmations\/([^/]+)(?:\/(decision))?$/,
  );
...
// HP5 P1-02 — generic confirmation control plane API.
//
// Frozen contract:
...
//
// HP5 first wave deliberately keeps `/confirmations` read-mostly. Live
// callers (PreToolUse permission, elicitation) populate rows via
// internal seams; the legacy `/sessions/{id}/permission/decision` and
// `/sessions/{id}/elicitation/answer` routes remain in place as compat
// aliases. Generic POST .../decision is exposed so future kinds and
// `/confirmations`-aware clients can submit decisions through the
// unified surface.
...
async function handleSessionConfirmation(
    ...
    if (statusParam && statusParam !== "any") {
      if (!isConfirmationStatus(statusParam) && statusParam !== "pending") {
        return jsonPolicyError(
          400,
          "invalid-input",
          "status must be one of pending|allowed|denied|modified|timeout|superseded|any",
          traceUuid,
        );
      }
  ...
  if (!isConfirmationStatus(status)) {
    return jsonPolicyError(
      400,
      "invalid-input",
      "status must be one of allowed|denied|modified|timeout|superseded",
      traceUuid,
    );
  }
  ...
  if (decisionPayloadRaw !== undefined && decisionPayloadRaw !== null) {
    if (
      typeof decisionPayloadRaw !== "object" ||
      Array.isArray(decisionPayloadRaw)
    ) {
      return jsonPolicyError(
        400,
        "invalid-input",
        "decision_payload must be a JSON object",
        traceUuid,
      );
    }
    ...
    if (statusParam && statusParam !== "any") {
      if (!isTodoStatus(statusParam)) {
        return jsonPolicyError(
          400,
          "invalid-input",
          "status must be one of pending|in_progress|completed|cancelled|blocked|any",
          traceUuid,
        );
      }
      filter = statusParam;
  ...
  if (route.kind === "create") {
    ...
    if (content.length === 0 || content.length > 2000) {
      return jsonPolicyError(
        400,
        "invalid-input",
        "content must be a non-empty string up to 2000 chars",
        traceUuid,
      );
    }
    ...
    const parentTodoUuid =
      typeof body.parent_todo_uuid === "string" ? body.parent_todo_uuid : null;
    if (parentTodoUuid !== null && !UUID_RE.test(parentTodoUuid)) {
      return jsonPolicyError(
        400,
        "invalid-input",
        "parent_todo_uuid must be a UUID when provided",
        traceUuid,
      );
    }
    try {
      ...
      if (err instanceof TodoConstraintError) {
        return todoConstraintToResponse(err, traceUuid);
      }
      throw err;
    }
  }
  ...
  if (route.kind === "patch") {
    ...
    if (content === undefined && status === undefined) {
      return jsonPolicyError(
        400,
        "invalid-input",
        "patch requires at least one of content|status",
        traceUuid,
      );
    }
    try {

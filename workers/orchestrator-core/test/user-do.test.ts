...
describe('NanoOrchestratorUserDO', () => {
  ...
  it('emits early session.replay.lost when ws attach last_seen_seq is ahead of relay cursor', async () => {
    const { created } = installFakePair();
    const { state, store } = createState();
    store.set(`sessions/${SESSION_UUID}`, {
      created_at: 'a',
    ...
    const response = await userDo.fetch(
      new Request(`https://orchestrator.internal/sessions/${SESSION_UUID}/ws?last_seen_seq=9`, {
        method: 'GET',
        headers: {
          upgrade: 'websocket',
    ...
    expect(frames[0]).toEqual(
      expect.objectContaining({
        ...
        degraded: true,
        trace_uuid: '55555555-5555-4555-8555-555555555555',
      }),
    );
    expect(frames.filter((frame) => frame.kind === 'event')).toEqual([]);
  });

...
describe("context-core shell smoke", () => {
  ...
  it("returns the shell worker identity", async () => {
    const response = await worker.fetch(new Request("https://example.com"), {
      WORKER_VERSION: "context-core@test",
    });
  ...
  it("embeds the built package manifest for context-core", () => {
    expect(NANO_PACKAGE_MANIFEST.worker).toBe("context-core");
  ...
  it("returns 401 binding-scope-forbidden for non-probe routes (ZX2)", async () => {
    ...
    expect(body.error.details.worker).toBe("context-core");

...
- ZX4 Phase 9 P9-01 翻转后标 `retired`（2026-04-28 完成，fast-tracked 7-day window per owner direction）:
  - `workers/orchestrator-core/src/user-do.ts` 中 `forwardStart / forwardStatus / forwardInternalJsonShadow` 已切到 RPC-only,fetch fallback 与 `jsonDeepEqual` parity 比较全部删除。
  - `workers/agent-core/src/host/internal.ts` `SUPPORTED_INTERNAL_ACTIONS` 已收紧为 `{ stream, stream_snapshot }`;`forwardHttpAction` helper 删除。
  - 5 leaf worker `workers_dev: false` 在 ZX2 P1-02 / R30 已落地,P9 阶段确认仍生效;旧 `*.workers.dev` URL 不可达。
  - 仅保留的两个 /internal/ 路由是 stream NDJSON 中继和 cursor-paginated snapshot,均无 dual-track parity 比较;保留是因为 stream 不是 RPC 友好的形态（无界字节流）。
- bash-core 段在 ZX2 Phase 3 P3-03/04 完成 RPC 化后已归并到 `nacp-internal`。
...
| ZX4 Phase 9 — `internal-http-compat` → `retired`(P3-05 flip + R31 + non-stream fetch handler deletion) | ✅ shipped 2026-04-28 |

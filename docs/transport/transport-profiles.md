...
| `session-ws-v1` | 对外 session 长连接 (`/sessions/{uuid}/ws`) | WebSocket + `NacpSessionFrameSchema` (基于 `NacpEnvelopeBaseSchema`) | JWT (query token) + `authenticateRequest` allowQueryToken | `@haimang/nacp-session` | active |
...
- frame：必须满足 `NacpSessionFrameSchema`（基于 `NacpEnvelopeBaseSchema` + `SessionFrameFields`）。
- close codes：见 `clients/api-docs/session-ws-v1.md`（ZX2 Phase 4 P4-05）。

**信任栈**：
- WS open：`authenticateRequest` allowQueryToken。
- 每 frame：`validateSessionFrame` + `assertSessionRoleAllowed` + `assertSessionPhaseAllowed`。

**契约源**：`@haimang/nacp-session`（ZX2 Phase 2 P2-03 扩 5 族 / 7 个新 message_type）。

**禁止**：
- 不允许在此 profile 上使用 flat `{ message_type, seq, body }` 而绕开 `NacpSessionFrameSchema`。
- compat 层只做 alias（旧 `{ kind, ... }` → frame.body），不引入新 envelope。
...
| `{ kind: '...', ... }` | server WS frame | Phase 4 P4-04 对齐到 `NacpSessionFrameSchema` |
...
- ZX2 Phase 2：在 `@haimang/nacp-core` 公开 `Envelope<T>` / `RpcMeta` / `NacpRpcError` / `validateRpcCall`；在 `@haimang/nacp-session` 落 5 族（7 个）新 message_type；扩 `@haimang/orchestrator-auth-contract` 为 facade-http-v1 单一源。
- ZX2 Phase 3：把 `internal-http-compat` 推向 `retired-with-rollback`。
- ZX3：评估是否升级 context-core / filesystem-core 为 RPC worker（决定后再加 profile）。

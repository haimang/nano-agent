...
> **Round-2 status**: `writeback-shipped` ✅ (converged with live data)
> **Writeback date**: 2026-04-20
> **Deploy**: `nano-agent-spike-do-storage-r2.haimang.workers.dev` (Version ID `c8e53de7-db35-45a5-955a-6598db49bc6f`)
> **Raw evidence**: `spikes/round-2-integrated/spike-do-storage-r2/.out/probe_follow-ups_do-size-cap-binary-search.json`

### Round-2 EXACT DO value cap (LIVE evidence, not projection)

| measurement | value |
|---|---|
| **lowBytes (last known-good)** | **2,199,424 bytes ≈ 2.0976 MiB** |
| **highBytes (first known-TOOBIG)** | **2,200,000 bytes ≈ 2.0981 MiB** |
| widthBytes | 576 |
| converged | `true` (1 KiB resolution) |
| attemptCount | 14 bisection steps |

**The true `state.storage.put()` value cap is ≈ 2.1 MiB.** The cap is
tight and sub-MiB-resolution narrow — not the 1–10 MiB bracket Round
1 was forced to leave.

### Round-2 verdict

Finding upgraded from `open` to `resolved`. Direct implication for B2
`@nano-agent/storage-topology`: `DOStorageAdapter.maxValueBytes`
(currently defaults to the conservative 1 MiB) can be raised to
**2,097,152 bytes (2 MiB even)** for a safety margin under the
measured 2.098 MiB cap. Anything ≥ 2.2 MiB MUST route via R2 promotion.

### Residual still-open

None. B8 worker-matrix can consume `2 MiB` as the safe authoritative
number.

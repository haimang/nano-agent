...
> 类型: `migration`(C1 + C2 slice 吸收 byte-identical)+ `modify`(WCA split + 共存期 duplicate 口径对齐)+ `new`(`workers/context-core/src/` 真实 runtime + context posture 决策 PR)
...
- **本次计划的直接产出**:
  ...
  - C2 slice 在 `workers/context-core` 建立 worker-side canonical copy,原 package 保留 coexistence duplicate(共存期)
...
│   │   └── context slice 保留 duplicate    [共存期;P5/D09 再清理]
...
- **[S4]** Phase 2:`packages/workspace-context-artifacts/src/index.ts` 保留原地 export + coexistence 注释,承认 context slice 在 workers/context-core 已有 canonical copy;不在 P3 强行改成跨 workers re-export
...
| 项目 | 判定 | 理由 | 预计何时重评 |
|------|------|------|--------------|
| `packages/workspace-context-artifacts` coexistence 策略 | `in-scope 保留 duplicate;不强行跨 workers re-export` | workspace boundary + 共存期 | P5 cutover 时 one-shot 切换 |
...
| P2-03 | packages coexistence 对齐 | `packages/workspace-context-artifacts/src/index.ts` 保留原地 export,并显式注明 context slice 已在 workers/context-core 建立 canonical copy;filesystem slice inline 继续 export;`evidence-emitters` artifact 部分继续 export | packages | coexistence duplicate truth 对齐 | `@nano-agent/workspace-context-artifacts` 的 public API 兼容 | 老 consumer 不破 |
...
> **注 P2-03 workspace boundary**:与 P1-03 同理,packages 内 re-export 指向 workers 通常禁止。P3 的实际收口口径即为 **保留 packages 内 context slice 原文件直到 P5 切换**;同时在 workers/context-core 保留 worker-side canonical copy,让 workers 侧 runtime 从 workers 拿,packages 消费者仍从 packages 拿(共存期纪律)。
...
- **Phase 目标**:5 context `.ts` + mixed helper context 部分迁入 context-core;filesystem 部分保留 in packages(P4 迁);packages duplicate 维持老 consumer 可用
...
2. C2 已吸收到 `workers/context-core/src/{context-layers,context-assembler,compact-boundary,redaction,snapshot}.ts`，并新增 `src/evidence-emitters-context.ts` 作为 context-only mixed-helper 落点；`packages/workspace-context-artifacts` 继续保留 coexistence duplicate。
...
5. `workers/context-core` probe 已从 W4 shell 口径升级为 `phase: "worker-matrix-P3-absorbed"` + `absorbed_runtime: true` + `library_worker: true`，对应 smoke test 已同步。
...
- **P3 in-scope 已闭合**：C1 / C2 吸收、mixed-helper context 切分、`appendInitialContextLayer` owner 归位、agent-core consumer 改线、probe truth 校准全部完成；WCA context slice 在本阶段按既定 workspace-boundary 决策保留 coexistence duplicate。

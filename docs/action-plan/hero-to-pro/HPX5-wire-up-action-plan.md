...
> 计划对象: `把已注册 schema 的 emitter / capability / runtime / 字节通道接通 + 修齐 18-doc pack 文档断点`
> 类型: `upgrade + add`
...
- `clients/api-docs/*.md` 18-doc pack 内引用 `workers/orchestrator-core/src/index.ts:1103-...` 这类行号,但 `workers/orchestrator-core/src/index.ts` 实际只有 18 行(已模块化到 `facade/routes/*.ts` + `*-control-plane.ts`),所有 implementation reference 均失效。
...
- **本次计划的直接产出**:
  ...
  - 18-doc pack 13 处契约不一致全修齐;implementation reference 全量刷新到模块化结构;`/start` 返回 `first_event_seq`;新增 `client-cookbook.md`
...
5. **Phase 5 — Docs Consistency + Reference Refresh(F7)**
   - **核心目标**:18-doc pack 内零契约不一致;implementation reference 全量刷新到 `workers/orchestrator-core/src/{facade/routes,*-control-plane}.ts`;新增 `clients/api-docs/client-cookbook.md` 收口 helper;`/start` 返 `first_event_seq`;`scripts/check-docs-consistency.mjs` 入 CI。
...
- **[S5]** F2c:`D1TodoControlPlane.create / update` 成功后,经 emit-helpers emit `session.todos.write` / `session.todos.update`(走 `messages.ts:369-393` 已冻 schema)。
...
- **[S9]** F7:18-doc pack 13 处契约修齐 + implementation reference 全量刷新 + 新增 `client-cookbook.md` + `/start` 返回 `first_event_seq` + `scripts/check-docs-consistency.mjs` 入 CI。
...
| 18-doc pack reference 行号刷新 | `in-scope` | F7 scope;deepseek §1.4 一手核查 `index.ts` 18 行 | never |
...
| P3-03 | Phase 3 | F2c todo emitter | update | `workers/orchestrator-core/src/todo-control-plane.ts`(在 create/update 后 emit) | row write 后经 emit-helpers emit `session.todos.write` / `.update` | medium |
...
| P5-01 | Phase 5 | 18-doc 13 处契约修齐 | update | `clients/api-docs/{auth,session,session-ws-v1,confirmations,permissions,context,workspace,todos,checkpoints,models,me-sessions,error-index,transport-profiles}.md` | 不一致全部对齐(详见 §5.5) | low |
| P5-02 | Phase 5 | implementation reference 全量刷新 | update | `clients/api-docs/*.md`(全部 18 份) | 行号刷新到 `facade/routes/*.ts` + `*-control-plane.ts` | low |
...
| P3-03 | F2c todo emitter | 在 `D1TodoControlPlane.create()` / `update()` 成功后,通过 service binding 让 session DO emit `session.todos.write` / `session.todos.update`(走 `messages.ts:369-393` 已冻 schema + emit-helpers F0 出口) | `workers/orchestrator-core/src/todo-control-plane.ts`, 服务绑定到 agent-core session DO | row write 后 ≤500ms WS 帧 emit | e2e:跑 todo create / update,WS 客户端必须收到帧 | F2c emit 与 F1 行为对称;polling 仍可用 |
...
| 编号 | 工作项 | 工作内容 | 涉及文件 / 模块 | 预期结果 | 测试方式 | 收口标准 |
|------|--------|----------|------------------|----------|----------|----------|
| P5-01 | 13 处契约修齐 | 详见 §5.5 修订清单(7 处来自 GPT §6.1–6.6 + 6 处来自 design D1–D7) | `clients/api-docs/{session,session-ws-v1,confirmations,permissions,context,workspace,todos,me-sessions,checkpoints,models,error-index,transport-profiles,auth}.md` | 18-doc pack 内零契约不一致 | `scripts/check-docs-consistency.mjs` 通过 | check 脚本通过 + 人工 review |
| P5-02 | implementation reference 刷新 | 18-doc pack 内所有 `workers/orchestrator-core/src/index.ts:NNN` 改为新模块化结构对应行号(如 `workers/orchestrator-core/src/facade/routes/session-bridge.ts:14-33` / `confirmation-control-plane.ts:96-131` 等) | `clients/api-docs/*.md`(全部 18 份) | 没有失效 reference | grep 全文 `index\.ts:[0-9]` 应为零 | 0 失效引用 |
...
| P5-05 | check-docs-consistency.mjs | 新建脚本检查:(a) 全部 18-doc pack 内的 confirmation kind 名是否统一为 `tool_permission`;(b) decision body 字段是否含 `status` + `decision_payload`(也允许 legacy `decision` + `payload` 但要标 deprecated);(c) `session_status` 7 值枚举一致;(d) `index\.ts:[0-9]+` 零出现;(e) `effective_model_id` 零出现(应统一为 `fallback_model_id`);(f) `tool_permission` vs `permission` kind 名一致 | `scripts/check-docs-consistency.mjs` (NEW), `package.json` `scripts.test:contracts` 添加调用 | CI fail-fast | `node scripts/check-docs-consistency.mjs` 本地通过 | CI 通过;Owner 复核 |
...
- **具体功能预期**:
  ...
  3. `todo-control-plane.ts` 在 create / update / delete 后 emit `session.todos.write` / `session.todos.update`(走 F0)
...
- **具体测试安排**:
  ...
  - **手动验证**:wrangler 本地起 agent-core + orchestrator-core,跑一个对话让 LLM 调 `write_todos`,前端 WS 收到 `session.todos.write`
- **收口标准**:
...
- **Phase 目标**:18-doc pack 内零契约不一致;CI 有自动化 gate。
...
- **具体功能预期 — 13 处契约修齐清单**:
  ...
  4. **decision body shape 统一**(GPT §6.4):全 18-doc 统一 `status + decision_payload`;legacy `decision + payload` 标 deprecated 且 façade dual-accept(实施时在 `session-control.ts` 的 confirmation decision handler 加 fallback)
...
- **收口标准**:
  ...
  - 18-doc 索引 → 19-doc(含 `client-cookbook.md`)
  - Owner 一次性复核通过
- **本 Phase 风险提醒**:
...
6. 18-doc pack 内零契约不一致;`/start` 返 `first_event_seq`;`client-cookbook.md` 收口前端实战兜底;`scripts/check-docs-consistency.mjs` 在 CI 通过。
...
- **回归测试**:
  - 既有 18-doc 描述的所有 happy path 行为(login / start / input / messages / cancel / close / files upload / todos HTTP CRUD / checkpoints / context probe / models)0 退化
...
5. 18-doc 内零契约不一致;`/start` 返 `first_event_seq`;`client-cookbook.md` live;CI gate 通过

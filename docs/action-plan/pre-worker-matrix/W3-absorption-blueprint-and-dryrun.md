...
采用 **先核对已存在文档，再做增量 revise，最后决定是否做 optional dry-run** 的方式推进。先 verify、后 patch、再可选验证，能避免 W3 重新膨胀成一次“提前吸收所有包”的新大阶段。

### 1.2 Phase 总览

| Phase | 名称 | 预估工作量 | 目标摘要 | 依赖前序 |
|------|------|------------|----------|----------|
| Phase 1 | reality verification pass | `S` | 核对已存在 template / pattern / map / blueprint 与 narrowed scope、W0 import truth 是否一致 | `W0-W2 design ready` |
| Phase 2 | 增量 revise 与 gap fix | `M` | 对已存在的 map / pattern / 2-3 份样本做必要修订 | `Phase 1` |
| Phase 3 | optional dry-run 与 closure | `M` | 按 owner 决定做 capability-runtime dry-run，或保留 placeholder | `Phase 2` |

### 1.3 Phase 说明

1. **Phase 1 — reality verification pass**
   - **核心目标**：确认已存在文档没有继续沿用旧 scope、旧 import path、旧 package 计数
   - **为什么先做**：W3 当前风险不是“还没写”，而是“已有文档与 reality 漂移”
2. **Phase 2 — 增量 revise 与 gap fix**
   - **核心目标**：对 map 与代表样本做增量修订，而不是重写
   - **为什么放在这里**：先 verify，再 patch，才能避免覆盖已完成设计资产
...
|------|------------|--------|------|------------------|------------|----------|
| P1-01 | Phase 1 | template reality 核对 | `update` | `TEMPLATE-absorption-blueprint.md` | 统一 blueprint 结构 | `low` |
| P1-02 | Phase 1 | pattern reality 核对 | `update` | `W3-absorption-pattern.md` | 统一迁移方法论 | `medium` |
| P1-03 | Phase 1 | W3 map reality 核对 | `update` | `W3-absorption-map.md` | 统一 package→worker 视图 | `medium` |
| P2-01 | Phase 2 | capability-runtime blueprint revise | `update` | `W3-absorption-blueprint-capability-runtime.md` | 代表 bash-core 吸收模式 | `medium` |
| P2-02 | Phase 2 | WCA split blueprint revise | `update` | `W3-absorption-blueprint-workspace-context-artifacts-split.md` | 代表 split-package 模式 | `high` |
| P2-03 | Phase 2 | session-do-runtime optional blueprint revise | `update` | `W3-absorption-blueprint-session-do-runtime.md` | 代表 host shell landing 模式 | `medium` |
...
### 4.1 Phase 1 — reality verification pass

| 编号 | 工作项 | 工作内容 | 涉及文件 / 模块 | 预期结果 | 测试方式 | 收口标准 |
|------|--------|----------|------------------|----------|----------|----------|
| P1-01 | template reality 核对 | 核对 blueprint 模板是否仍符合 narrowed W3 与当前执行口径 | `TEMPLATE-absorption-blueprint.md` | 后续 blueprint 可直接 copy-fill | 文档 review | 模板稳定 |
| P1-02 | pattern reality 核对 | 核对 owner/test/deprecation/partial discipline 是否与当前仓库事实一致 | `W3-absorption-pattern.md` | 迁移方法不再散落 | 文档 review | generic rules 完整 |
| P1-03 | map reality 核对 | 核对 `9 packages / 10 units`、worker 归宿与 W0 import truth | `W3-absorption-map.md` | worker 归宿不再反复争论 | 文档 review | map 清晰 |

### 4.2 Phase 2 — 增量 revise 与 gap fix

| 编号 | 工作项 | 工作内容 | 涉及文件 / 模块 | 预期结果 | 测试方式 | 收口标准 |
|------|--------|----------|------------------|----------|----------|----------|
| P2-01 | capability-runtime blueprint revise | 以现有蓝图为基线修正 representativeness 说明：`cross-package dep = 0`，代表性来自 fake-bash/policy seam 与 LOC 体量，而不是循环依赖复杂度 | `W3-absorption-blueprint-capability-runtime.md` | bash-core 吸收路径清晰 | 文档 review | fake-bash / typed runtime 纪律明确 |
| P2-02 | WCA split blueprint revise | 校准 context/filesystem split 样本与当前 worker 归宿、consumer path | `W3-absorption-blueprint-workspace-context-artifacts-split.md` | split-package 路径清晰 | 文档 review | mixed helper 处理清晰 |
| P2-03 | session-do-runtime blueprint revise | 校准 host shell optional 样本与 `agent-core` 的 DO shell reality | `W3-absorption-blueprint-session-do-runtime.md` | agent-core host shell landing 清晰 | 文档 review | 与 A1/A2-A5 边界不冲突 |
...
### 5.1 Phase 1 — reality verification pass
...
### 5.2 Phase 2 — 增量 revise 与 gap fix
...
- **本 Phase 修改文件**：
  ...
  - `docs/design/pre-worker-matrix/W3-absorption-map.md`
- **具体功能预期**：
  1. bash-core 有代表样本，但其 representativeness 明确来自 fake-bash/policy seam 与大体量代码，不被误写成“跨包依赖最复杂”
...
- **本 Phase 风险提醒**：
  ...
  - capability-runtime dry-run 不能代表循环依赖 pattern，需要在文档中诚实写明
...
- **具体功能预期**：
  ...
  2. 若不做，也不能把 W3 判失败
  3. 若做 dry-run，要显式承认 `capability-runtime` 约 `src 5000 LOC / test 4500 LOC`，工作量不再按 `S` 级口径估算
  4. closure 需诚实写明哪种状态达成
- **具体测试安排**：
...
- **A**：`是。session-do-runtime 继续作为 optional representative 样本，不升级为 W3 硬 gate。`
...
| capability-runtime 代表性被误读 | 容易把它误写成“跨包依赖最复杂样本” | `medium` | 明确写出 `cross-package dep = 0`，其价值来自 fake-bash/policy seam 与 LOC 体量 |
...
1. 已存在的 map / template / pattern / blueprint 全部完成 reality verification pass
2. 至少 2-3 份代表性 blueprint 完整
3. W3 不再声称需要 full package absorb
4. optional dry-run 状态被诚实记录，且若执行会附 LOC / 工作量事实
5. capability-runtime 的 representativeness 被正确表述
6. worker-matrix P0 可据此直接进入实施拆解

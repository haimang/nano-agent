...
- **本次计划解决的问题**：
  ...
  - Worker / V8 isolate 下不能依赖真实 bash、真实进程与真实宿主 FS，因此最小命令面必须显式映射到 virtual workspace 与受控 fetch / TS 执行
...
- **本次计划的直接产出**：
  ...
  - 以最小命令集 `pwd/ls/cat/write/rg/curl/ts-exec` 跑通 fake workspace 场景与 progress/cancel 路径
...
本 action-plan 分 **5 个 Phase**，执行策略是 **“先 capability 真相，再 fake bash 映射，再 target dispatch，最后用最小命令集和场景测试收口”**。  
...
| Phase 5 | Command Surface / 测试 / 文档 / 收口 | M | 以 just-bash 可迁移映射面为基线完成命令层收口并验证 | Phase 1-4 |
...
3. **Phase 3 — Policy Gate / Execution Targets / ToolCall Bridge**
   ...
   - **为什么放在这里**：治理与 target dispatch 是 capability runtime 的运行时心脏，必须先于最小命令集落地。
...
5. **Phase 5 — Command Surface / 测试 / 文档 / 收口**
   - **核心目标**：以 just-bash 可迁移映射面为基线完成命令层收口，并用 fake workspace、fake service-binding、fake policy 场景证明架构成立。
...
- **风险控制原则**：不把完整 POSIX shell、真实进程管理、真实 git runtime 引入 v1；命令集与 execution target 必须显式收窄
- **测试推进原则**：先测 registry / planner / gate / executor，再用最小命令集做 fake workspace integration；progress/cancel/oversized result 必须有回归测试
...
- **[S13]** just-bash-compatible command surface 的仓内重写与 capability 映射（至少先覆盖 `pwd`、`ls`、`cat/read`、`write`、`rg`、`curl/fetch`、`ts-exec`，并以可迁移面为基线继续补齐）
...
| P5-01 | Phase 5 | command surface pack | `add` | `src/capabilities/*.ts`、`src/fake-bash/commands.ts` | 以 just-bash 可迁移映射面为基线完成命令层收口 | high |
...
| 编号 | 工作项 | 工作内容 | 涉及文件 / 模块 | 预期结果 | 测试方式 | 收口标准 |
|------|--------|----------|------------------|----------|----------|----------|
| P5-01 | minimal capability pack | 落地 `pwd/ls/cat/write/rg/curl/ts-exec` 与 virtual git subset seam | `src/capabilities/*.ts` | v1 最小工作流可跑通 | smoke tests | 最小命令面足以支撑 session 骨架验证 |
...
- **具体测试安排**：
  ...
  - **手动验证**：对照 `vpa-fake-bash-by-GPT.md` 的最小命令集与不支持清单
- **收口标准**：
  - 命令外形与能力真相完全分离
  - v1 最小命令集可被 registry 明确枚举
...
- **本 Phase 新增文件**：
  ...
  - `packages/capability-runtime/test/integration/minimal-pack-smoke.test.ts`
- **本 Phase 修改文件**：
  - `packages/capability-runtime/README.md`
  - `packages/capability-runtime/src/index.ts`
- **具体功能预期**：
  1. 命令面至少要先支撑读写 workspace、搜索文本、抓取 URL、执行受限 TS 脚本，并以 just-bash 可迁移映射面为后续补齐基线。
  2. `rg` 明确采用 TS namespace scan 或 service-binding search worker 的降级路径，而非承诺真实 ripgrep 二进制。
...
- **为什么必须确认**：最小命令集直接决定 fake bash 的暴露面、测试范围与 prompt 约束。
- **当前建议 / 倾向**：`v1 正式冻结为 pwd/ls/cat/write/rg/curl/ts-exec + virtual git subset seam`
- **Q**：`v1 是否正式确认最小命令集只覆盖 pwd/ls/cat/write/rg/curl/ts-exec，并把 git 只作为接口占位处理？`
- **A**：v1 完整实现 just-bash 的移植工作，支持 just-bash 提供的全部映射面

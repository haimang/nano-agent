...
- **本次计划的直接产出**：
  ...
  - `agent-core` `/internal/sessions/*` 最小可用 contract（至少 `start/cancel/stream`）
...
- **风险控制原则**：`只实现 narrow start/cancel/stream baseline，不提前铺满 F2/F3 全量 surface`
...
- **[S3]** 为 `agent-core` 增加最小 `/internal/sessions/:id/{start,cancel,stream}` contract 与 secret gate
- **[S4]** 打通 `agent -> orchestrator` first event NDJSON relay，并给出最小 live 证据

### 2.2 Out-of-Scope（本次 action-plan 明确不做）

- **[O1]** 完整实现 `input/verify/status/timeline/ws/reconnect`
...
| P2-02 | Phase 2 | per-user DO registry shell | `add` | `workers/orchestrator-core/src/user-do/*` | façade 获得最小持久 owner | `high` |
| P3-01 | Phase 3 | agent internal route family 起步 | `update` | `workers/agent-core/src/index.ts` `workers/agent-core/src/host/internal/*` | `/internal/start/cancel/stream` 真正存在 | `high` |
...
| P2-02 | user DO registry shell | 实现 `user_uuid` owner、`session_uuid` mint、最小 `SessionEntry` 写入 | `src/user-do/*` | façade 不再是无状态转发层 | worker tests | user DO 能保存 minted session |

### 4.3 Phase 3 — internal binding 最小落地

| 编号 | 工作项 | 工作内容 | 涉及文件 / 模块 | 预期结果 | 测试方式 | 收口标准 |
|------|--------|----------|------------------|----------|----------|----------|
| P3-01 | `/internal/*` 最小集 | 在 `agent-core` 实现 `start/cancel/stream` 三条 internal path，走 index.ts 早退，不污染 legacy parser | `workers/agent-core/*` | internal contract 真正存在 | agent-core tests | 不再依赖 legacy public path |
...
| P4-02 | cursor 初始语义 | 对齐 `relay_cursor = last_forwarded.seq`，首次 forward 后写入 entry | `user DO registry` | D3/D5 语义不再停留在文档 | integration tests | cursor 与 frame seq 一致 |
...
- **本 Phase 修改文件**：
  ...
  - `workers/orchestrator-core/src/ingress/*.ts`
- **具体功能预期**：
  1. `/internal/start/cancel/stream` 三条 path 存在
  2. secret gate 与 typed 401 存在
  3. `validateInternalAuthority()` 在 parse 后、DO fetch 前运行
- **具体测试安排**：
  - **单测**：`agent-core internal routing/auth tests`
  - **集成测试**：`orchestrator -> agent internal start/cancel`
...
- **具体功能预期**：
  ...
  2. cursor 写入遵守 `last_forwarded.seq`
  3. first event 可被 façade 层观察到
- **具体测试安排**：
...
- **具体功能预期**：
  ...
  2. minimal session-start live test 可证明 first roundtrip 成立
  3. closure 写明哪些内容仍留给 F2
- **具体测试安排**：
  - **单测**：`无新增`
...
Q1 / Q2 / Q5 等 F1 硬前置答案已在 `FX-qna.md` 冻结，F1 应直接消费这些答案实施。
...
- **技术前提**：`F0 已 closure，D1/D2/D3/D5/D6 与 FX-qna 为 F1 SSOT`
...
- **基础校验**：
  ...
  - `agent-core internal start/cancel/stream 可被断言`
- **单元测试**：

...
采用 **先集中 policy helper，再落 tenant truth law，再接 executor recheck seam，最后补 negative tests 与 closure** 的方式推进。F4 不创造新的业务域，只把合法性判断从“分散事实”变成“中央层事实”。

### 1.2 Phase 总览

| Phase | 名称 | 预估工作量 | 目标摘要 | 依赖前序 |
|------|------|------------|----------|----------|
| Phase 1 | policy helper 落位 | `L` | public/internal legality 统一入口 | `F3 closed` |
...
| P2-01 | Phase 2 | `TEAM_UUID` bootstrap law | `update` | `workers/*/wrangler.jsonc` `src/index.ts` | preview/prod tenant truth 实化 | `high` |
...
| P3-02 | Phase 3 | executor recheck seam | `update` | `workers/bash-core/src/executor.ts` | future truth recheck 有落点 | `medium` |
| P4-01 | Phase 4 | negative tests | `add/update` | package-e2e / worker tests | law 有负例证据 | `medium` |
| P4-02 | Phase 4 | F4 closure | `add` | `docs/issue/orchestration-facade/F4-closure.md` | F4 可正式审计 | `low` |
...
| 编号 | 工作项 | 工作内容 | 涉及文件 / 模块 | 预期结果 | 测试方式 | 收口标准 |
|------|--------|----------|------------------|----------|----------|----------|
| P2-01 | `TEAM_UUID` bootstrap law | 在 preview/prod 配置中显式提供 `TEAM_UUID`，入口处做 misconfigured deploy check | `workers/*/wrangler.jsonc` `src/index.ts` | tenant truth 成为真实 deploy law | worker tests | preview/prod 缺失不再默默 `_unknown` |
...
| P3-02 | executor recheck seam | 在 `CapabilityExecutor` 前放集中 hook，先做 simple pass-through / deny | `workers/bash-core/src/executor.ts` | future truth recheck 有正式落点 | unit tests | seam 存在且不破坏当前执行流 |

### 4.4 Phase 4 — negative tests 与 closure

| 编号 | 工作项 | 工作内容 | 涉及文件 / 模块 | 预期结果 | 测试方式 | 收口标准 |
|------|--------|----------|------------------|----------|----------|----------|
| P4-01 | negative tests | 覆盖 missing trace、missing authority、tenant mismatch、preview missing TEAM_UUID、escalation | package-e2e / worker tests | law 有负例 proof | tests | 关键 reject paths 全绿 |
| P4-02 | F4 closure | 汇总 F4.A 完成证据，明确未做的 domain 范围 | `docs/issue/orchestration-facade/F4-closure.md` | F4 闭合且不越权 | 文档 review | closure 可直引 |
...
- **具体功能预期**：
  ...
  2. executor 前存在集中 hook
  3. future credit/quota 无需重开 executor 主路径
- **具体测试安排**：
...
- **技术前提**：`F3 已完成 canonical cutover，public owner 已单一`
- **运行时前提**：`可修改 5 个 worker 的 wrangler truth 与相关 tests`
...
5. `F4-closure.md` 已明确 F4.A 完成、F4.B 未做`

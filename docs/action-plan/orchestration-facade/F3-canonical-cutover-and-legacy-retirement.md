...
F3 只迁 **session-facing public suites / harness / docs**。它不把 `context-core` / `filesystem-core` 当成 public session edge 迁移对象；这两个 worker 继续保留 internal posture / probe-only 角色，本周期不碰它们的 HTTP surface。
...
| P2-02 | Phase 2 | auth-negative / legacy-negative cases | `add` | `orchestrator-core/06-07*` | canonical suite 具备必要负例，且 JWT 层与 authority 层职责分离 | `medium` |
...
| P4-01 | Phase 4 | legacy HTTP `410` | `update` | `workers/agent-core/*` | old session HTTP paths honest retire（覆盖 7 个 action，含 `end`） | `high` |
| P4-02 | Phase 4 | legacy WS `426` | `update` | `workers/agent-core/*` | old WS path 不再升级 | `high` |
| P5-01 | Phase 5 | cutover closure + probe rollover | `add/update` | `docs/issue/orchestration-facade/F3-closure.md` `workers/orchestrator-core/src/index.ts` | F3 完整闭合，并把 orchestrator probe marker bump 到 F3 | `low` |
...
| 编号 | 工作项 | 工作内容 | 涉及文件 / 模块 | 预期结果 | 测试方式 | 收口标准 |
|------|--------|----------|------------------|----------|----------|----------|
| P4-01 | legacy HTTP `410` | 让 `agent-core /sessions/:id/{start,input,cancel,end,status,timeline,verify}` 返回 typed `410` + canonical hint | `workers/agent-core/*` | old HTTP surface honest retire | negative tests | all 7 legacy HTTP asserts updated |
...
- **具体功能预期**：
  1. session-facing package-e2e 全部归 orchestrator-core，F1 `01/02` 在此被增强/替换，不保留双份
...
- **具体功能预期**：
  ...
  2. orchestrator probe marker 同步 bump 到 `orchestration-facade-F3`
  3. 为 F4/F5 提供 clear proof
- **具体测试安排**：
  - **单测**：`无`
  - **集成测试**：`无新增`
...
- **收口标准**：
  ...
  - orchestrator probe marker 已切到 `orchestration-facade-F3`
- **本 Phase 风险提醒**：
...
4. `agent-core` legacy HTTP 7 个 session actions（含 `end`）返回 typed `410`，legacy WS 返回 typed `426`

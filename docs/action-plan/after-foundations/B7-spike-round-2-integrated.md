...
> 计划对象：`the historical round-2 integrated spikes tree` 下的 round-2 spike workers、round-2 finding/status writeback、B7 closure issues
> 类型：`spike`
> 作者：`GPT-5.4`
> 时间：`2026-04-20`
> 文件位置：
> - `the historical round-2 integrated spikes treeREADME.md` （new）
> - `the historical round-2 integrated storage spike workspace/{package.json,tsconfig.json,wrangler.jsonc,README.md,.gitignore}` （new）
> - `the historical round-2 integrated storage spike workspace{worker,result-shape}.ts` （new）
> - `the historical round-2 integrated storage spike workspace` （new）
> - `the historical round-2 integrated storage spike workspace{kv-cross-colo-stale,do-size-cap-binary-search,curl-high-volume,r2-concurrent-put}.ts` （new）
> - `the historical round-2 integrated storage spike workspace{storage,bash,context}.ts` （new）
> - `the historical round-2 integrated storage spike workspace{deploy,run-all-probes,extract-finding}.ts|.sh` （new）
> - `the historical round-2 integrated binding spike workspace{package.json,tsconfig.json,wrangler.jsonc,README.md,.gitignore}` （new）
> - `the historical round-2 integrated binding spike workspace{worker,follow-ups,re-validation,result-shape}.ts` （new）
> - `the historical round-2 integrated binding spike workspace{package.json,tsconfig.json,wrangler.jsonc,README.md,.gitignore}` （new）
> - `the historical round-2 integrated binding spike workspace{worker,handlers}.ts` （new）
> - `the historical round-2 integrated binding spike workspace{deploy-both,run-all-probes,extract-finding}.ts|.sh` （new）
...
> - `docs/the historical round-2 integrated spikes tree**/*.md` （new：仅当 Round 2 暴露新增 `integrated-F*` finding）
...
- **本次计划的直接产出**：
  - **D1**：建立 `the historical round-2 integrated spikes tree` 的 deploy-shaped 代码与脚本骨架
...
| Phase 2 | Round-2 spike skeleton + isolated deploy surface | 建 `the historical round-2 integrated spikes tree` 目录、separate workers/resources、result/evidence shape、package imports | Phase 1 |
...
- **[S2]** 建立 `the historical round-2 integrated spikes tree` 代码与 deploy skeleton
...
|---|---|---|---|---|---|---|
| P1-01 | Phase 1 | 对齐 B1 / P6 / B2-B6 / spike reality | check | docs/** + historical spike artifacts + packages/** | 冻结 B7 单一真相 | high |
| P1-02 | Phase 1 | 冻结 verdict / status transition / owner gate | decision | finding docs + closure docs | 避免 B7 口径漂移 | high |
| P1-03 | Phase 1 | 冻结 round-2 naming / resource isolation / phase closure grouping | decision | historical spike artifacts + docs/issue/** | baseline 与 round-2 解耦 | medium |
| P2-01 | Phase 2 | 建 `spike-do-storage-r2` skeleton | add | `the historical round-2 integrated storage spike workspace/**` | storage/b3/b4 seam 可部署复验 | high |
| P2-02 | Phase 2 | 建 `spike-binding-pair-r2` skeleton | add | `the historical round-2 integrated binding spike workspace/**` | binding/b5/b6 seam 可部署复验 | high |
...
| P5-01 | Phase 5 | round-2 discipline check + raw evidence pack | doc | docs/historical spike artifacts + .out/** | round-2 自身可 audit | medium |
...
|---|---|---|---|---|---|---|
| P1-01 | 输入对齐核查 | 把 `B1-final-closure`、`B1-handoff`、P6 design r2、B2-B6 action-plan 当前口径、Round 1 spike 目录与 package.json reality 对成唯一执行清单 | `docs/**`, `the historical round-1 bare-metal spikes tree**`, `packages/**` | B7 不再混用旧 5-item follow-up 口径 | 人工核对 + grep | follow-up 数、closure 条件、资源范围全部单一化 |
...
| P1-03 | round-2 命名与隔离冻结 | 固定 `-r2` worker naming、separate resource names、以及 `B7-phase-1/2/3 + final` closure grouping | historical spike artifacts + `docs/issue/after-foundations/**` | baseline 与 round-2 不混淆 | 文档核对 | naming / resources / issue naming 全部统一 |

### 4.2 Phase 2 — Round-2 spike skeleton + isolated deploy surface

| 编号 | 工作项 | 工作内容 | 涉及文件 / 模块 | 预期结果 | 测试方式 | 收口标准 |
|---|---|---|---|---|---|---|
| P2-01 | `spike-do-storage-r2` 骨架 | 新建 package metadata、wrangler profile、README、`.gitignore`、`src/worker.ts`、`IntegratedProbeDO`、follow-up/re-validation 子目录 | `the historical round-2 integrated storage spike workspace/**` | storage round-2 worker 可独立部署 | `deploy:dry-run` | worker skeleton 与 Round 1 一样 deploy-shaped，但导入 shipped packages |
| P2-02 | `spike-binding-pair-r2` 骨架 | 新建 worker-a-r2 / worker-b-r2 及 scripts，保留 caller/callee 分工 | `the historical round-2 integrated binding spike workspace/**` | binding-pair round-2 worker 可独立部署 | `deploy:dry-run` | 两个 worker 与 scripts 可一起组装运行 |
...
| P4-03 | finding status transition | 更新 13 required + 2 optional original finding docs 的 `§0 status` 与 round-2 closure section；若新增问题独立写成 `integrated-F*` docs，并链接 downstream issue | `docs/historical spike artifacts`, `docs/the historical round-2 integrated spikes tree**` | B1 finding 真正从“待验证”变成 closure verdict | 文档核对 | 没有只更新 rollup、不更新原 finding 的半套做法 |

### 4.5 Phase 5 — Discipline / docs / closure / handoff

| 编号 | 工作项 | 工作内容 | 涉及文件 / 模块 | 预期结果 | 测试方式 | 收口标准 |
|---|---|---|---|---|---|---|
| P5-01 | round-2 discipline check | 产出 `docs/spikes/_DISCIPLINE-CHECK-round-2.md`，按 7 条纪律逐项自检；其中 §3.7 明确记录 “Round 2 允许 import shipped packages” 的例外 | docs/historical spike artifacts | Round 2 自身也可 audit，不只是 probe 结果可看 | 文档核对 | 6/7 strict + 1 modified nuance 被说清 |
...
1. `the historical round-2 integrated spikes tree` 目录树与 2 组 round-2 worker skeleton 已建立
...
- **B7 首批交付建议**：
  - 先建 `the historical round-2 integrated spikes treeREADME.md` + 两套 Wrangler skeleton（`spike-do-storage-r2`、`spike-binding-pair-r2`）把 `deploy:dry-run` 可执行面立起来，再实作 follow-ups / re-validation probes。
...
- **storage-r2 骨架**（`the historical round-2 integrated storage spike workspace/`）：
...
- **binding-pair-r2 骨架**（`the historical round-2 integrated binding spike workspace/`）：
...
**`the historical round-2 integrated spikes tree` 根**：
- `README.md`

**`the historical round-2 integrated storage spike workspace/`**（14 项）：
- `package.json`
- `tsconfig.json`
- `wrangler.jsonc`
- `README.md`
...
**`the historical round-2 integrated binding spike workspace`**（9 项）：
- `package.json`
- `tsconfig.json`
- `wrangler.jsonc`
- `README.md`
...
**`the historical round-2 integrated binding spike workspace`**（11 项）：
- `package.json`
- `tsconfig.json`
- `wrangler.jsonc`
- `README.md`
...
**`the historical round-2 integrated binding spike workspace`**（3 项）：
- `deploy-both.sh`
- `run-all-probes.sh`
- `extract-finding.ts`
...
`docs/handoff/after-foundations-to-worker-matrix.md` 的 `§2 / §7` 与 `docs/issue/after-foundations/B8-phase-1-closure.md` 的 `§3 / §4` 现已正式消费本节 LIVE evidence，作为 worker-matrix phase 的直接输入。

...
|------|------|------------|----------|----------|
| Phase 1 | Wave B Schema | `M` | 落 `nano_sessions / turns / messages / contexts / activity_logs` 及索引 | `Z1 closed` |
...
- **[S1]** 落 Wave B D1 schema：`nano_sessions / nano_session_turns / nano_session_messages / nano_session_contexts / nano_session_activity_logs`
...
|------|------------|--------|------|------------------|------------|----------|
| P1-01 | Phase 1 | wave-B migrations | `add` | `workers/orchestrator-core/migrations/002-session-truth-and-audit.sql` | 把 session durable truth 建出来 | `high` |
...
|------|--------|----------|------------------|----------|----------|----------|
| P1-01 | wave-B migrations | 依据 ZX-D1 落 session / turn / message / context / activity tables 与索引 | `workers/orchestrator-core/migrations/002-session-truth-and-audit.sql` | Z2 durable truth 有真实 D1 落点 | migration smoke / schema review | tables、foreign keys、indexes、enum discipline 完整存在 |

### 4.2 Phase 2 — Public Session Durable Truth

| 编号 | 工作项 | 工作内容 | 涉及文件 / 模块 | 预期结果 | 测试方式 | 收口标准 |
|------|--------|----------|------------------|----------|----------|----------|
| P2-01 | public session persistence | 让 `start/input/history/timeline/verify` 把 session truth 落到 D1，并从 D1 回读 | `workers/orchestrator-core/src/index.ts` | public session façade 具备 durable owner 身份 | package-e2e / cross-e2e | `history/timeline/verify` 不再只看 transient state |
...
| 编号 | 工作项 | 工作内容 | 涉及文件 / 模块 | 预期结果 | 测试方式 | 收口标准 |
|------|--------|----------|------------------|----------|----------|----------|
| P3-01 | DO hot-state compaction | 让 DO 只保留 `activeWsBindings`、`lastReplayCursor/seq`、`latestTransportMeta`、`pendingSinceLastCheckpoint` | `workers/orchestrator-core/src/user-do.ts` `workers/agent-core/src/host/do/nano-session-do.ts` | hot-state 与 Q6 冻结答案一致 | package-e2e / state assertions | 不再把 transcript/context 真相长时间留在 DO memory |
| P3-02 | replay / heartbeat baseline | 建 replay cursor、heartbeat ack、`every 10m + alarm` snapshot/recover path | `user-do.ts` `ws-controller.ts` `checkpoint.ts` | reconnect / hibernation 恢复可证明 | package-e2e / cross-e2e | reconnect 后 history/replay 可继续，alarm 恢复不丢主状态 |

### 4.4 Phase 4 — RPC Kickoff

| 编号 | 工作项 | 工作内容 | 涉及文件 / 模块 | 预期结果 | 测试方式 | 收口标准 |
|------|--------|----------|------------------|----------|----------|----------|
| P4-01 | internal status RPC smoke | 先把 `status` 做成 WorkerEntrypoint RPC smoke，建立 caller/callee envelope truth | `workers/agent-core/src/host/internal.ts` `workers/orchestrator-core/src/index.ts` | internal RPC-first seam 开始成为现实 | package-e2e / smoke | `status` 可经 RPC 返回真实 session state |
| P4-02 | internal start kickoff | 让 `start` 也开始走 RPC-first seam，并保持 public façade contract 不变 | `internal.ts` `session-edge.ts` `routes.ts` | internal HTTP glue 开始退居兼容层 | package-e2e / cross-e2e | public `start` 与 internal `start` 共享同一 durable truth |

### 4.5 Phase 5 — Audit Closure

| 编号 | 工作项 | 工作内容 | 涉及文件 / 模块 | 预期结果 | 测试方式 | 收口标准 |
|------|--------|----------|------------------|----------|----------|----------|
| P5-01 | replay/activity tests | 覆盖 reconnect、replay cursor、history/timeline readback、activity append-only 负例 | `test/package-e2e/orchestrator-core/**` `test/cross-e2e/**` | Z2 truth 有回归护栏 | `pnpm test:package-e2e` / `pnpm test:cross` | replay / timeline / activity 都有 green proof |
| P5-02 | Z2 closure | 写 `Z2-closure.md`，说明 Wave B schema、DO hot-state、replay、RPC kickoff 的交付状态 | `docs/issue/zero-to-real/Z2-closure.md` | Z3 可以直接消费 Z2 baseline | 文档 review | closure 列出 durable truth 与已知 residuals |
...
- **具体功能预期**：
  1. session / turn / message / context / activity truth 都有 durable 落点。
  2. append-only activity log 与 transcript truth 分层明确。
  3. 后续 replay / heartbeat / context snapshot 不再缺 durable owner。
- **具体测试安排**：
  - **单测**：`无`
...
- **具体功能预期**：
  ...
  3. status/timeline 不再只看 process-local state。
- **具体测试安排**：
...
- **具体功能预期**：
  1. DO hot-state 收敛到 Q6 冻结的 4 组集合。
  2. replay cursor/heartbeat/ack 具备 first-wave runtime reality。
  3. `every 10m + alarm` checkpoint 能把 pending delta 写回 durable truth。
- **具体测试安排**：
...
- **具体功能预期**：
  ...
  2. `start` 开始共享 RPC-first internal path。
  3. 外部 façade contract 不变，但内部 boundary 更清晰。
- **具体测试安排**：
...
- **收口标准**：
  - `status` 经 RPC 返回真实状态
  - `start` 至少完成 first-wave kickoff
  - no-escalation / envelope truth 仍成立
- **本 Phase 风险提醒**：

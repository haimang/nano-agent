...
- **[S3]** `Live Session Stream / Durable Audit Trace / Durable Transcript` 三分法与分类 helper
- **[S4]** `TraceSink` 接口
...
- **[S14]** `metric-names` / evidence constants：沿用 codex 风格层级命名，但不引入完整 exporter
...
| P1-02 | Phase 1 | trace taxonomy | `add` | `src/trace-event.ts`、`src/classification.ts`、`src/types.ts` | 冻结 durable/live/transcript 三层真相 | high |
...
| P1-02 | trace taxonomy | 定义 base fields、evidence extensions、audience 与三层分类 | `src/trace-event.ts`、`src/classification.ts`、`src/types.ts` | live/audit/transcript 边界明确 | schema 单测 | 无混淆 durable 与 live 的事件 |
| P1-03 | truncation / metric names | 定义输出截断策略与 `agent.turn.* / agent.tool.* / agent.api.*` 命名基线 | `src/truncation.ts`、`src/metric-names.ts` | evidence 命名与大小边界有统一真相 | 单测 | 没有散落命名与任意大 payload |
...
- **具体功能预期**：
  ...
  3. 输出截断与 metric naming 拥有统一基线。
- **具体测试安排**：
  - **单测**：schema、classification、truncation、metric name constants
  - **集成测试**：无
  - **回归测试**：classification matrix 快照

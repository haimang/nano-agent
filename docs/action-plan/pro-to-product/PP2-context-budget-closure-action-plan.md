...
│   ├── workers/context-core/src/control-plane.ts（canonical budget owner）
...
|------|------------|--------|------|------------------|------------|----------|
| P1-01 | Phase 1 | Budget owner freeze | `update` | `workers/context-core/src/control-plane.ts`, `workers/agent-core/src/host/runtime-mainline.ts`, `workers/agent-core/src/llm/request-builder.ts` | 防止 probe/runtime 使用两套预算算法 | `medium` |
...
|------|--------|----------|------------------|----------|----------|----------|
| P1-01 | Budget owner freeze | 冻结 `workers/context-core/src/control-plane.ts` 的 `resolveBudget()` 为 canonical budget owner；agent-core preflight 优先复用同一计算语义，仅在无法直接复用时才抽取纯函数 helper，并保留 parity tests | context-core / agent-core | probe 与 runtime 同语义 | unit tests | 同一输入得到同一 needCompact/headroom |
...
- **具体功能预期**：
  ...
  3. canonical budget owner 默认留在 `context-core`；若后续必须抽纯函数 helper，也只能作为该 owner 的实现细化，不得引入第二套 truth。
- **具体测试安排**：
...
- **具体功能预期**：
  ...
  2. `tokens_before/tokens_after`、protected fragments（至少 `<model_switch>` / `<state_snapshot>`）、compacted/kept message count 可读取。
...
- **具体功能预期**：
  ...
  2. prompt mutation 证据的 primary capture point 冻结在 `runtime-mainline.ts` 构造 `exec` 并调用 `gateway.executeStream(exec)` 之前；必要时在 `llm/gateway.ts` / `request-builder.ts` 暴露 test-only snapshot seam，而不是只靠 `compact.notify` 反推。
...
- **具体功能预期**：
  ...
  3. compact latency alert 按 PP0 evidence shape 与 `latency_alert.threshold_key / exceeded_count / accepted_by_owner / repro_condition` 登记。
- **具体测试安排**：
  - **单测**：无。
...
- **技术前提**：确认 `workers/orchestrator-core/migrations/` 当前最新编号仍为 `017`；若仓库已出现 `018+`，需先与 owner 确认下一可用编号后再触发任何 D1 例外。
...
- 需要同步更新的设计文档：
  ...
  - 若实现期发现 design/QNA 与代码事实冲突，必须先在本 action-plan 或 `PP2-closure.md` 记录发现，再判断是否回到 `PPX-qna.md` 补充 / 修订答案，并同步通知 PP3 / PP5 / PP6。
- 需要同步更新的说明文档 / README：
  - `docs/issue/pro-to-product/PP2-closure.md`
  - `clients/api-docs/context.md`（必要最小同步）
- 需要同步更新的测试说明：

...
- `network-egress.test.ts` 逐条列出 14 个私有地址并断言 `curl-private-address-blocked` marker；scheme 四条 negative case（file/ftp/gopher/data）断言 `curl-scheme-blocked` marker。**澄清（A8-A10 review Kimi R3）**：`isPrivateHost()` 是 **deny-list 语义**，不是 allow-list —— 任何未命中 deny-list 的公网 hostname（如 `api.github.com`、`example.com`、`registry.npmjs.org` 等）都会被接受。测试里仅对 `example.com` 做正向断言，只是为了证明「非私有地址可通过」，并不构成 "only example.com" 的白名单约束。

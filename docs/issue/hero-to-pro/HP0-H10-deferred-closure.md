...
| HP6-D2 | filesystem-core snapshot/restore/copy-to-fork RPC (`readSnapshot / writeSnapshot / copyToFork / cleanup`) | `workers/filesystem-core/src/index.ts:202-280` 4 个新 RPC 方法 + `buildSnapshotKey` 锚定 `tenants/{team}/sessions/{session}/snapshots/{checkpoint}/{normalized}` 路径律 | filesystem-core 300/300 pass |

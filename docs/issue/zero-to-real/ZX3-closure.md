...
### 1.3 Phase 3 — keep-set posture freeze(v2 simplified;详见 ZX3 plan §16.1-§16.2)
...
- P4-02: 5 个 surviving guardians 迁 `test/root-guardians/`;14 broken retire;`test-command-coverage` 重写守护新 scripts
- P4-03: `package.json` scripts 切换 — `test:contracts` 指向 `test/root-guardians/*.test.mjs`;`test:legacy:*` 删除
- P4-04: cross-e2e 01/02/03/07/10/11 + zx2-transport + `live.mjs` 全部改为 facade-唯一-entry 模型(承接 ZX2 R30)
...
- **Stream-1**: deploy-only bug(R28+R29)+ 7 天 parity 观察 + P3-05 翻转 + R31 workers_dev URL 撤销

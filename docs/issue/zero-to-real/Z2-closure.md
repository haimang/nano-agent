...
1. 新增 `workers/orchestrator-core/migrations/002-session-truth-and-audit.sql`，落下：
   - `nano_conversations`
...
1. `input/cancel/timeline/verify/stream` 仍主要走 fetch-backed internal seam；Z2 只要求 `status` smoke 与 `start` kickoff，不要求一次性全切 RPC。
2. `nano_session_activity_logs` 当前已经 append-only、具备 redaction wrapper 与关键索引，但 query/analytics plane 仍是后续运营阶段主题，不属于 Z2。
3. root `pnpm test:contracts` 仍被仓库既有的 `docs/nacp-session-registry.md` 缺失阻塞；这不是 Z2 本轮引入的问题，因此未被伪装成 Z2 blocker。
4. agent-core 仍未直接 bind `NANO_AGENT_DB`；Z2 的 durable truth owner 仍在 orchestrator façade / D1 repository，而不是下沉到 runtime worker 自己读写 D1。

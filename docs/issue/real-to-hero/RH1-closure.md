...
> **RH1 阶段闭合**:hook / permission / elicitation / usage 4 条 lane F side-channel 从 contract-only 升级为可观察 live 的 wire(scheduler emits hook_emit / runtime delegate routes to HookDispatcher / emit*Request 真发 frame via cross-worker push topology / onUsageCommit live / `/usage` strict snapshot zero-shape)。RH2(GET /models + GET /sessions/{id}/context)可在 RH1 已 wire 的 ORCHESTRATOR_CORE service binding + `forwardServerFrameToClient` RPC + WorkerEntrypoint default export 上施工。
...
| Phase 2 — Permission/Elicitation Frame Emit (P1-03 + P1-04) | ✅ closed | `nano-session-do.ts` emit*RequestAndAwait 在 await 前 push frame via `pushServerFrameToClient` |
| Phase 3 — Cross-Worker WS Push RPC (P1-06a + P1-06b + P1-07) | ✅ closed | `agent-core/wrangler.jsonc` 新增 ORCHESTRATOR_CORE binding;`orchestrator-core/src/entrypoint.ts` 新建 default export `WorkerEntrypoint` 暴露 `forwardServerFrameToClient` RPC;`orchestrator-core/src/user-do.ts` 新增 `__forward-frame` 内部路由;`agent-core/src/host/env.ts` SessionRuntimeEnv 新增 ORCHESTRATOR_CORE 类型 |
| Phase 4 — Usage Push Live (P1-08) | ✅ closed | `nano-session-do.ts` `onUsageCommit` 接 `pushServerFrameToClient(session.usage.update)` |
...
| Phase 6 — Cross-Worker E2E + Preview Smoke (P1-13) | ✅ closed | preview deploy `orchestrator-core` Version `34cfc8a6...` + `agent-core` Version `de2fd54f...`;6 worker `/debug/workers/health` `live: 6 total: 6`;business chain smoke pass(register → start → usage zero-shape)。3 个 e2e (P1-10/11/12) 单元测试覆盖 wire 正确性,真投递 round-trip 由 RH3 D6 IngressAuthSnapshot.user_uuid 落地后接续 |

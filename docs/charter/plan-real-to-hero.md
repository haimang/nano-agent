...
> - `r3, 2026-04-29 — 追加 §16 RHX1 supplemental phase：冻结 post-RH6 的 DDL SSOT 整理、README/index/test 导航更新、RH0-RH6 deferred sweep 三类专项收敛范围与退出条件`
>
> **直接输入包（authoritative）**：
...
## 16. 附加阶段 RHX1 — DDL SSOT / Docs & Index Hygiene / Deferred Sweep

> `RHX1` 是 **post-RH6 supplemental phase**，不是重开 `real-to-hero` 主产品范围；它只做专项收敛：把 DDL 组织成可读的 SSOT、把 worker/package/test 文档与索引补到当前代码现实、并扫一轮 RH0-RH6 已知遗留项，能在本范围内闭合的就闭合，不能闭合的在 RHX1 closure 里说明原因。

### 16.1 一句话目标

> **让仓库在 RH6 之后拥有“可读的 schema 真相、可导航的工程文档、可追踪的 deferred ledger”，避免进入下阶段前再次因 migration 漂移、README 过期、测试入口不清而反复返工。**

### 16.2 RHX1 In-Scope

1. **DDL SSOT 整理**
   - 扫描 `workers/orchestrator-core/migrations/*.sql` 全量 DDL。
   - 直接把 **当前 `migrations/` 目录本身** 整理成按业务簇可读的 SSOT：清理碎片、合并不和谐部分、统一 SQL 头部与格式。
   - 不再额外创建 `ssot/` 子目录或 README；SSOT 就是整理后的当前 SQL 集合本身。
2. **README / index / test navigation 更新**
   - 更新全部已存在的 `workers/*/README.md` 与 `packages/*/README.md`，把 stale 内容与当前代码现实对齐。
   - 使用树状结构展示当前目录、职责、入口文件、验证命令。
   - 补齐 test 导航文件，明确 contracts / package-e2e / cross-e2e / worker-local tests 的定位与入口。
3. **RH0-RH6 deferred sweep**
   - 扫描 `real-to-hero` 既有 deferred / carry-over / blocked 项。
   - 能在 RHX1 范围内解决的继续解决；不能解决的必须在 RHX1 closure 里写明 **为什么不解决、落到哪里、是否仍 blocker**。

### 16.3 RHX1 Out-of-Scope

1. 改写 remote 已应用 migration 历史，或用 RHX1 重新分配 RH0-RH6 的 migration 编号。
2. 重新打开 RH1-RH6 的产品范围（新 endpoint / 新 worker / 新 provider / manual evidence 采集）。
3. 任何需要 owner 设备侧参与才能完成的 manual evidence。
4. hero-to-platform 级别的大平台设计（admin plane / billing / broader multi-deploy / second provider）。

### 16.4 RHX1 退出条件

1. `workers/orchestrator-core/migrations/` 当前 SQL 集合本身已按业务簇整理、统一格式，并能作为 DDL SSOT 直接阅读。
2. 全部现有 `workers/*/README.md`、`packages/*/README.md` 已与当前代码事实对齐；test 导航文件已建立并可引导测试。
3. RHX1 closure 文档存在，并包含：
   - 之前 deferred / carry-over 的完整台账；
   - 哪些在 RHX1 内解决了；
   - 哪些仍未解决；
   - 每个未解决项的原因与下游落点。
4. RHX1 的文档与整理工作不引入新的 schema / contract / test drift。

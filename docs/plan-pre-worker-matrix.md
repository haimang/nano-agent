...
> **r2 说明**:GPT review 盲点 2 + §1.5 调整后 — publishing 降为 **parallel track**:pipeline skeleton 必须在 pre-phase 就绪,但首次真实发布可 parallel 到 worker-matrix first-wave 期间完成。worker-matrix first-wave **允许** workers 用 `workspace:*` 作 interim 依赖。
...
15. **首次发布可选**:`nacp-v1.4.0` tag 可在 pre-phase 末期 push 并触发发布;**也可**延到 worker-matrix first-wave 期间(owner 决定)
16. **Dogfood 可选**:若首次发布在 pre-phase 完成,则做 throwaway 消费者;若首次发布延到 worker-matrix,dogfood 也顺延
...
29. 第一次真实发布:tag `nacp-v1.4.0`,CI 以 `nacp-core@1.4.0` 为 bundle anchor 发布 nacp-core 1.4.0 + nacp-session 1.3.0(或后续实际版本)
30. Dogfood 消费者 — **建议路径**:
...
3. 真实发布的 `@<scope>/nacp-core@1.4.0` + `@<scope>/nacp-session@1.3.0`(或各自当时的实际版本)
...
5. **W4 不依赖 W2 首发**(r2 调整):W4 workers 首版可用 `workspace:*`;W2 pipeline skeleton 就绪即可;真实首发可 parallel / 延后
...
3. **import / publish 策略已冻结**:
   ...
   - 首次发布可在 pre-phase 末期 OR parallel 到 worker-matrix — owner 决定时机

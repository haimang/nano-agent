...
> - **r2 (2026-04-21)— post-GPT-review narrowing**:依据 `docs/plan-pre-worker-matrix-reviewed-by-GPT.md` 的 7 盲点裁决与 §9 response 清单,对本 charter 做 scope 收窄:(a) 新增 §0.5 "Long-term vs First-wave 分层原则";(b) §1.5 publishing 决策:时机从硬 blocker 改为 parallel track(是否做不变,何时做改);(c) §3 一句话目标重写;(d) §4.1 In-Scope / §4.2 Out-of-Scope 整体重整,协议代码化降级为 RFC-only,4 worker deploy 收为 1 real + 3 dry-run,10 blueprint 收为 map + 2-3 detailed;(e) §8 时长从 4-5 周降为 2-3 周;(f) §11 exit criteria 从 11 条收窄为 6 条。净 scope 降幅约 60%,RFC 文档化加强约 80%。
> - **r3 (2026-04-23)— post-W2 publish truth sync**:基于 `@haimang/nacp-core@1.4.0` + `@haimang/nacp-session@1.3.0` 已首发成功的事实,统一 W2 相关表述、引用与示例路径;保留 r2 的“parallel 非 blocker”原则,但把当前真相层从“未发布 / deferred”刷新为“已发布 / dogfood 已验证”。
>
> **输入依据:**
...
Owner 指出:4 个 first-wave worker 应该像 the historical spikes tree 一样拥有 **`workers/` 顶级目录**,每个 worker 有独立的 `wrangler.jsonc` / `package.json` / `src/` / `test/`,从而获得 Cloudflare-native 的部署隔离、风险隔离、独立生命周期。
...
> **在 after-foundations 与 worker-matrix 之间,插入一个专门处理"拓扑冻结 + 包策略冻结 + import/publish 策略冻结 + 3 个跨 worker orphan 决策落地 + 最小必要脚手架 + 方向性 RFC"的窄前置阶段;这一阶段不做任何业务能力的 worker,不实装新跨 worker 协议,不强制 publishing 先于 worker-matrix 启动,只冻结 worker-matrix 起跑线所需的最小决策与脚手架。**

简称:

> **Pre-Worker-Matrix: Topology Freeze + Minimal Scaffold + Directional RFC**

### 0.5 Long-term vs First-wave 分层原则(r2 新增,post-GPT-review)

GPT review 最深刻的洞见是:**原 r1 把"长期吸收终局"与"first-wave 验证路径"混成一层**,把 remote-era 的未来议题(新协议代码化、全面发布、10 份 blueprint 实装、4 次真实 deploy)当成 first-wave 硬前置,导致 pre-phase 从"校准起跑线"膨胀成"再做一轮 foundations"。

r2 必须严格分层:

| 层 | 职责 | 证据驱动 | 本 phase? |
|---|---|---|---|
| **Layer 1:终局拓扑**(长期) | `workers/` 独立目录;packages 逐步被吸收;NACP 对外发布 | owner 决策即可冻结 | **pre-phase 冻结拓扑决定 + 最小脚手架,不做大规模实装** |
| **Layer 2:first-wave 验证**(中期) | 把已经存在的 substrate 装成 live agent turn loop;4 workers 内组装 | owner 决策 + 已有 substrate | **worker-matrix phase** — 不是 pre-phase |
| **Layer 3:remote split / 协议扩张 / 全面吸收**(远期) | γ workspace RPC / β remote compact / 10 包全量搬迁 / 全面 publishing | live loop 运行证据 | **worker-matrix 后续 phase** — 由 live loop 证据驱动 |

这一分层是 r2 所有 scope 收窄的指导原则。**若某项工作属 Layer 2 或 Layer 3,绝不前移到 pre-phase。**
...
5. **`workers/` 与 `packages/` / the historical spikes tree / `context/` 平级**,每个 worker 有独立项目结构
...
### 1.5 NACP 必须真实发布到 GitHub Packages(Owner 决策,r2 调整时机)

11. NACP 两个包**必须**以 GitHub Packages 形式对外发布(决策不变)
12. 发布流水线、auth 配置、publish-on-tag 纪律、dogfood 验证仍在本阶段 W2
13. **r2 调整**:发布时机**不是** worker-matrix 启动的硬 blocker,而是 **parallel track**:
    - worker-matrix first-wave 期间,`workers/*` 的 `package.json` 可以用 `workspace:*` 作为 interim 依赖
    - Publishing pipeline 必须在 pre-phase 完成 skeleton(publishConfig + workflow + auth 配置)
    - 首次真实发布可以发生在 pre-phase 末期,也可以 parallel 到 worker-matrix first-wave 期间
    - Worker-matrix 末期(或某个 owner-指定 milestone)前,workers/ 必须切换到 GitHub Packages path(从 workspace:* 替换为真实版本号);这一切换动作属 worker-matrix scope,不属 pre-phase
14. **当前事实(r3 补记)**:W2 已按 owner-aligned scope 完成首次发布:
    - `@haimang/nacp-core@1.4.0`
    - `@haimang/nacp-session@1.3.0`
    - tag:`nacp-v1.4.0`
    - run:`https://github.com/haimang/nano-agent/actions/runs/24814439569`
- 本调整依据:GPT review 盲点 2 + owner §9.2 决策
...
1. **NACP 当前 published surface**
   - `@haimang/nacp-core 1.4.0` — Layer 6 matrix + `NacpErrorBodySchema` + W0 absorbed vocabulary
   - `@haimang/nacp-session 1.3.0` — session profile matrix + `SessionStartInitialContextSchema`
   - 两包已发布到 GitHub Packages,并已通过同仓 dogfood consumer install/build/smoke 验证
...
| **NACP 发布路径曾缺失（现已由 W2 收口）** | 现已有 `publish-nacp.yml`、双包 `publishConfig`、`nacp-v1.4.0` 成功 run 与 dogfood published-path evidence | **W2 completed** |
...
## 3. 本阶段的一句话目标(r2 重写,post-GPT-review)

> **在 after-foundations 交付的 NACP 1.3 契约基础上,本阶段完成 5 件窄工作:(a) 把**少量**真正跨 worker 的 shape 吸收进 `nacp-core`(W0 narrower)— 不搬 runtime class,只搬 wire vocabulary + 相邻 helper;(b) 为 γ workspace RPC / β compact delegate / cross-worker evidence forwarding 3 条协议方向**撰写 RFC**(W1 RFC-only),**不**在本阶段实装代码;(c) 建立 GitHub Packages publishing **pipeline skeleton**(W2)— 首次发布可 parallel 到 worker-matrix,**不阻塞** first-wave 启动;(d) 写 1 份 Tier B 10 包的 **absorption map**(轻)+ 2-3 份代表性 detailed blueprint(W3 narrower)— llm-wrapper dry-run 降为 optional;(e) 建立 `workers/` 顶级目录 + 4 个空 worker 项目脚手架 + **agent-core 1 次真实 deploy + 其他 3 worker `wrangler deploy --dry-run`**(W4 narrower);产出物是 worker-matrix 的窄起跑线:**拓扑冻结 + 包策略冻结 + import 策略冻结 + 3 个 orphan 决策落定 + 最小必要脚手架 + 方向性 RFC 就绪**。**

**r2 较 r1 变化**:
- W0 边界收窄(BoundedEvalSink class 不搬,hooks catalog 只搬 wire shape)
- W1 整体 downgrade(code-ship → RFC-only)
- W2 降级(parallel 不 blocker)
- W3 收窄(map + 2-3 代表 blueprint,dry-run 可选)
- W4 收窄(1 real deploy + 3 dry-run)
- 总时长:4-5 周 → 2-3 周
...
> **r2 note**:原 r1 的 In-Scope 过宽。r2 依据 GPT review 7 盲点 + §0.5 分层原则,对 A-F 整体收窄。下面是 **r2 narrower 版**。

#### A. NACP 协议吸收(W0 narrower)

1. **【r2 拆分】** `BoundedEvalSink` **class 本身不搬**(它是 runtime implementation,留 `session-do-runtime`;最终随 agent-core absorption 进入 workers/agent-core)。只搬**协议-adjacent vocabulary**:
   - `EvalSinkEmitArgs`、`EvalSinkOverflowDisclosure`、`EvalSinkStats` 类型 shape
   - "dedup on envelope messageUuid" 的 contract 文档
   - `extractMessageUuid` helper(因为它读 envelope shape,属协议 adjacent)
2. 把 `CrossSeamAnchor` header law 从 `session-do-runtime/src/cross-seam.ts` 搬到 `nacp-core/src/transport/cross-seam.ts`(**只搬 propagation 部分**;failure taxonomy + startup queue 留原位)
3. 把 evidence vocabulary(`assembly / compact / artifact / snapshot` Zod schema)从 `workspace-context-artifacts/src/evidence-emitters.ts` 搬到 `nacp-core/src/evidence/vocabulary.ts`(emit 函数留原位)
4. **【r2 拆分】** 把 hooks **wire-level schema**(event name union + payload Zod schema)从 `hooks/src/catalog.ts` 搬到 `nacp-core/src/hooks-catalog/`;**不搬** `HookEventMeta` 的 runtime metadata(`blocking / allowedOutcomes / redactionHints` — 这些是 dispatch runtime 语义)
5. 把 storage key / ref / tenant law(builders + `_platform/` 常量)从 `storage-topology/src/{keys,refs}.ts` 搬到 `nacp-core/src/storage-law/`;`NacpRefSchema` 不迁(已在 envelope.ts)
6. 所有原位置改为**re-export from nacp-core**,保证消费者 1 次 minor diff 即可迁移
7. Ship `nacp-core 1.4.0`; `nacp-session` 仅在真实引入新的 published surface 时才跟随 bump，否则保持 `1.3.0`

#### B. 跨 worker 协议方向 RFC(W1 RFC-only,r2 major downgrade)

> **r2 说明**:原 r1 要求本阶段实装 3 条跨 worker 新协议代码(NACP message families + helper functions)。GPT review 盲点 4 + §0.5 分层原则明确:**这些协议属 Layer 3 远期工作,应由 live loop 证据驱动**,不是 first-wave 硬前置。r2 将 B 整体降级为 **RFC-only**:本阶段只写协议方向 RFC,不实装代码。

8. **γ workspace service-binding 协议 RFC**:
   - 写 `docs/rfc/nacp-workspace-rpc.md`:描述未来 remote filesystem.core 时所需的最小 RPC 面
   - **不**在本阶段实装 Zod schema / matrix registration / helper
   - RFC 作为未来 owner-driven phase 启动时的 input
9. **β remote compact delegate 文档化**:
   - 写 `docs/rfc/remote-compact-delegate.md`:确认现有 `context.compact.request/response` 足够承载 kernel ↔ context.core 跨 worker 的 delegate 用途
   - **不**新增 message types(现有 family 足够)
   - **不**实装 `createRemoteKernelCompactDelegate` helper(worker-matrix 阶段 live 后再实装)
10. **Cross-worker evidence forwarding RFC**:
    - 写 `docs/rfc/evidence-envelope-forwarding.md`:规定 evidence record 通过 `audit.record` envelope 转发的 pattern
    - **不**实装 `wrapEvidenceAsAudit` / `extractEvidenceFromAudit` helper(W0 shipped evidence vocabulary + audit.record 已存在的契约已足够;helper 实装延后)
11. 本阶段**不** ship 任何跨 worker 协议代码 — 3 份 RFC 冻结方向,代码化属 worker-matrix 后续 phase

#### C. GitHub Packages 发布 pipeline skeleton(W2,r2:parallel 非 blocker)

> **r2/r3 说明**:publishing 作为 worker-matrix 的 hard blocker 被取消这一点不变;但当前事实已经从“可选延后”升级为“已真实完成首发”。因此本节既保留 `workspace:*` interim 的合法性,也明确当前 published path 已经存在。
...
15. **首次发布已完成(r3)**:`nacp-v1.4.0` 已完成成功发布,当前 published path 为 `@haimang/nacp-core@1.4.0` + `@haimang/nacp-session@1.3.0`
16. **Dogfood 已完成(r3)**:`the retired historical dogfood consumer` 已从 GitHub Packages install/build/smoke 成功
17. 写 `docs/design/pre-worker-matrix/W2-publishing-discipline.md`:publish-on-tag 纪律、版本号规则、紧急撤回流程、**workspace:\* 切换到 published version 的迁移 checklist**(new)

#### D. Tier B 吸收 Map + 代表 Blueprint(W3 narrower,r2:map + 2-3 代表 + dry-run 可选)

> **r2 说明**:GPT review 盲点 5-6 — 10 份 detailed blueprint 是 scope 膨胀;llm-wrapper 代表性低。r2 改为:1 份 map(必)+ 2-3 份高复杂度代表 detailed blueprint(必)+ dry-run(可选,若做则选 capability-runtime 更代表性)。

18. 写 1 份 **`docs/design/pre-worker-matrix/W3-absorption-map.md`**:
    - 10 × (source package → destination worker) 总映射表(轻粒度,不逐文件)
    - 每行含:粗粒度 LOC / 主要跨包依赖标记 / deprecated 时机 policy
19. 对 **2-3 份** 代表性高复杂度包产 detailed blueprint(优先级:`capability-runtime` + `workspace-context-artifacts` split + 可选 `session-do-runtime`)
20. **Dry-run 可选**(r2 降级 — **非 exit gate**):
    - 若 owner 决定做 dry-run,**不选 llm-wrapper**(代表性低);改选 `capability-runtime`
    - 若不做 dry-run,pattern spec 由未来 worker-matrix P0 首次 absorb 时回写
21. 其他 7 份 detailed blueprint 留到 worker-matrix charter r2 / P0 阶段按需撰写(不是 pre-phase gate)

#### E. `workers/` 脚手架与 Cloudflare 部署验证(W4 narrower,r2:1 real + 3 dry-run)

> **r2 说明**:GPT review 盲点 1 — 4 次真实 deploy 过度。r2 改为 **1 real deploy(agent-core)+ 3 worker dry-run(wrangler deploy --dry-run)**。DevOps 验证 1 次已够。

22. 建 `workers/` 顶级目录
23. 为 4 个 worker 各建(完整脚手架保留):
    - `workers/agent-core/wrangler.jsonc` — 含预期 DO binding + SERVICE bindings
    - `workers/agent-core/package.json` — **interim 可用 `workspace:*`,发布就绪后切 GitHub Packages version**(r2 调整)
    - `workers/agent-core/src/index.ts` — hello-world fetch handler
    - `workers/agent-core/test/smoke.test.ts` — 壳级 smoke test
    - 其他 3 个 worker 同构
24. **agent-core 做 1 次真实 `wrangler deploy --env preview`** 到 Cloudflare(1 个 URL),验证:
    - wrangler 配置解析成功
    - NACP 包 resolve 成功(若 W2 已发布用 GitHub Packages;否则 workspace:\*)
25. **其他 3 workers(bash-core / context-core / filesystem-core)做 `wrangler deploy --dry-run`**:仅验证 wrangler.jsonc 配置合法性,不真实部署
...
25. 写 `docs/handoff/pre-worker-matrix-to-worker-matrix.md`:列出 **r2 narrower 4 就绪(拓扑冻结 / 包策略冻结 / import-publish 策略冻结 / scaffold 就绪)+ 3 份方向 RFC + 2-3 份代表 blueprint + absorption map** + worker-matrix 阶段可直接消费的 input pack
...
W4 的 workers 脚手架**优先**消费当前已经存在的 GitHub Packages published path,但不把 `workspace:*` interim 路径视为错误。

- ❌ 错误:把 “尚未发布” 当作 W4 的默认前提,继续沿用旧的 deferred 叙事
- ✅ 正确:承认 `@haimang/nacp-core@1.4.0` + `@haimang/nacp-session@1.3.0` 已存在,并让 W4 在 published path 与 `workspace:*` interim 两条合法路径中显式选择其一

这也意味着 W2 已经为 W4 提供了真实 published baseline；而 r2 的并行原则仍然成立：若某个 worker shell 先走 `workspace:*`，也不构成对 charter 的违背。
...
1. `pnpm --filter @haimang/nacp-core build` 通过
...
25. `packages/nacp-core/package.json` + `packages/nacp-session/package.json` 加 `publishConfig`:
    ...
    ```
26. owner scope 已确定为 `@haimang`，本 phase 不再保留 `@nano-agent` 占位
...
29. 第一次真实发布已完成:tag `nacp-v1.4.0`,CI 以 `nacp-core@1.4.0` 为 bundle anchor 发布 `@haimang/nacp-core@1.4.0` + `@haimang/nacp-session@1.3.0`
30. Dogfood 消费者 — **建议路径**:
    - 方案 A(推荐):在本仓库新建 `the retired historical dogfood consumer`,独立 package.json,从 GitHub Packages import nacp-core + nacp-session,build + test 成功即可删除
...
3. 真实发布的 `@haimang/nacp-core@1.4.0` + `@haimang/nacp-session@1.3.0`
...
35. 调整 import:从 `@haimang/nacp-core` 等走 GitHub Packages(当前已可用)
...
39. 估算:若做 capability-runtime dry-run,基于 LOC + tests 预估其他包吸收时长(作为 worker-matrix P0 现场 blueprint 起草的 reference)

#### 交付物(r2 narrower)

1. **1 份 absorption map**(`W3-absorption-map.md`)— 10 行 package → worker 表
2. **2-3 份代表性 detailed blueprint**(capability-runtime + workspace-context-artifacts-split + 可选 session-do-runtime)
3. **Optional** `capability-runtime` dry-run(若做搬进 `workers/bash-core/src/`;若不做,pattern 延到 P0 回写)
4. `docs/design/pre-worker-matrix/W3-absorption-pattern.md`(pattern doc;若无 dry-run 则 generic 部分 + placeholder)
5. `docs/issue/pre-worker-matrix/W3-closure.md`

#### 收口标准(r2 narrower)

1. absorption map 完整 + owner glance-pass
2. 2-3 份代表 blueprint 满填 TEMPLATE 8 节
3. Pattern doc generic 部分(deprecated wording / 共存期纪律)shipped;具体 pattern(循环引用 / LOC 系数)可留 placeholder
4. 7 份未写 detailed blueprint 在 W3 closure memo 里交代给 worker-matrix P0 现场撰写(不作为 W3 gate)
...
41. 4 个 worker 项目,每个包含:
    ...
    - `package.json` — from GitHub Packages import `@haimang/nacp-core` + `@haimang/nacp-session`;其他 dependencies 先空着
...
3. **agent-core 1 个 URL 外网可访**(或 owner-gated);其他 3 workers `wrangler deploy --dry-run` pass
4. CI workflow run 1 次 + green(matrix 4 workers 全绿)
5. 证明 NACP resolve 成功:(i) 若 W2 已发首发,从 GitHub Packages pull 到 agent-core bundle;(ii) 若未发,用 `workspace:*` resolve + build 成功
...
48. `docs/handoff/pre-worker-matrix-to-worker-matrix.md`:
    - r2 narrower 4 就绪清单(拓扑 / 包策略 / import-publish 策略 / scaffold;对应 charter §11 primary 6 条的前 5 条)
...
1. **W0 必须最先**:W1(RFC)/ W2 / W3 都需要 nacp-core 的最终 wire shape
2. **W2 与 W1 可并行**(r2 调整):W2 pipeline skeleton 构建基于 W0 产物(1.4.0)即可;W1 是 RFC-only,不产 shipped symbol,首发无需等 W1
3. **W3 map + 代表 blueprint 可与 W1 / W2 并行**:blueprint 与 map 是 doc,不依赖代码完成
4. **W3 dry-run(optional)依赖 W4 `workers/bash-core/` 目录**(r2 调整,因 dry-run 目标从 llm-wrapper 改为 capability-runtime):若 owner 决定做 dry-run,capability-runtime 吸收到 `workers/bash-core/src/`,该目录由 W4 建立;因此 W3 dry-run 部分必须在 W4 目录就绪后执行
5. **W4 不依赖 W2 首发**(r2 调整)这一原则仍成立;但当前 reality 已是 `@haimang/*` 首发完成,因此 W4 若选择 published path 可直接消费真实版本
6. **W5 最后**:收口必须等全部 phase 完成

### 8.4 估计时长(r2 narrower)

| Phase | 时长 | 依赖 | 累计(带并行) |
|---|---|---|---|
| W0 | 0.5-1 周(5 类 wire-level 吸收 + 全 regression;narrower:BoundedEvalSink class 不搬 + hooks metadata 不搬) | — | 1 周 |
| W1 | 0.5 周(**RFC-only** × 3 份;无代码实装) | W0 | 1 周(与 W0 并行尾段) |
| W2 | 0.5 周(pipeline skeleton;首发可延) | W0(可与 W1 并行) | 1-1.5 周 |
| W3 | 0.5-1 周(map + 2-3 代表 blueprint;dry-run 可选,若做 +0.3 周) | W0 + W1(可与 W4 并行) | 1.5-2 周 |
| W4 | 0.5 周(脚手架 + 1 real deploy + 3 dry-run + CI) | W2(skeleton 够即可) | 1.5-2 周 |
| W5 | 0.5 周 | 全部 | 2-2.5 周 |

**总时长:2-2.5 周**(含缓冲可达 3 周)。r2 narrower 较 r1 的 4-5 周缩短 ~50%。比 after-foundations(5-6 周)明显短,与 worker-matrix 预计时长相仿 — 符合 "窄前置阶段" 定位。
...
9. **W2 的 tag 纪律**:只有 `nacp-v*.*.*` 格式 git tag 触发发布,不用 branch 或 commit；当前已验证 `nacp-v1.4.0`
...
| W2 dogfood consumer build | dogfood 目录 | 从 GitHub Packages import `@haimang/nacp-core@1.4.0` + `@haimang/nacp-session@1.3.0` 成功 |
...
### 11.1 Primary Exit Criteria(r2 narrower — 6 条,严格对应 GPT §6 建议)

> **r2 说明**:原 r1 11 条 primary 条目导致 pre-phase 变成新大阶段。r2 按 GPT §6 建议收窄至 **6 条硬 exit 条件**,其余原 r1 条目或退为 optional / 或移到 out-of-scope / 或退为 secondary outcome。

1. **目录拓扑已冻结**:
   - `workers/` 顶级目录已建立;4 workers 目录命名已 owner-approved
   - wrangler 文件归属规则已写清(`workers/<name>/wrangler.jsonc`)
2. **包策略已冻结**:
   - `nacp-core` + `nacp-session` 是唯一永久对外 2 包已正式书面确认
   - Tier B 吸收映射表(`W3-absorption-map.md`)已存在
3. **import / publish 策略已冻结**:
   - 明确 worker-matrix first-wave 期间可用 `workspace:*`(r2 parallel 决策)
   - GitHub Packages pipeline skeleton 就绪(publishConfig + workflow + auth)
   - 当前已存在真实 published path:`@haimang/nacp-core@1.4.0` + `@haimang/nacp-session@1.3.0`
4. **3 个关键 orphan 决策已冻结**:
   - `initial_context` host consumer 归属 agent.core(已在 worker-matrix plan §6.3 冻结;本 phase 确认)
   - capability remote/local policy(CAPABILITY_WORKER 默认 local 还是 remote — 本 phase charter 明确)
   - filesystem first-wave 是否保持 host-local truth(本 phase charter 明确)
5. **最小 worker scaffold 已存在**:
   - 至少 1 个样板 worker(agent-core)可以 build + test + 真实 `wrangler deploy --env preview`
   - 其他 3 个 worker 脚手架就绪 + `wrangler deploy --dry-run` pass
   - 方向性 RFC × 3 shipped(workspace-rpc / remote-compact-delegate / evidence-forwarding)
6. **worker-matrix r2 起跑线已重写清楚**:
   - `plan-worker-matrix.md` 顶部 deprecated banner 解除,state 改 `needs-rewrite-r2`
   - `docs/handoff/pre-worker-matrix-to-worker-matrix.md` shipped,含 r2 rewrite 所需 input pack

### 11.2 Secondary Outcomes(结果非锚点)

- `nacp-core` 1.3.0 → 1.4.0(additive,W0 narrower 范围)
- `nacp-session` 按需 1.3.0 → 1.4.0 或保持
- `plan-worker-matrix.md` 状态从 deprecated → needs-rewrite-r2
- 若 owner 选择做 dry-run:`workers/bash-core/src/...` 有 capability-runtime 局部代码(样本)
- 若 owner 选择 pre-phase 首发 NACP:GitHub Packages 出现 1.4.0
...
- primary 1-6 任一未满足
- W0 吸收过程破坏 1.3.0 消费者(re-export 失效)
- B7 LIVE 契约 tests 红
- agent-core 真实 deploy 失败且未 resolved

### 11.4 明确**从原 r1 移除 / 降级**的 exit items

下列条目在 r1 属 primary exit,r2 改为 optional / parallel / out-of-scope:

- ❌ "3 条新协议**代码**落地" → 改为 "3 份 RFC shipped"(W1 RFC-only)
- ❌ "NACP 真实发布 + dogfood 必须完成" → 改为 "publishing pipeline skeleton 就绪,首发可延到 worker-matrix"(W2 parallel)
- ❌ "10 份 absorption blueprint" → 改为 "map + 2-3 份代表"(W3 narrower)
- ❌ "llm-wrapper dry-run 成功作为 gate" → 改为 optional + 若做选 capability-runtime(W3 narrower)
- ❌ "4 workers 真实部署 4 次" → 改为 "1 real(agent-core)+ 3 dry-run"(W4 narrower)
- ❌ "6 份 closure docs + 1 handoff memo" → 保留但收口粒度轻量化(W5 narrower)

这些收窄确保 pre-phase 不再膨胀成新大阶段。
...
基于本阶段产出的 **r2 narrower "4 就绪"起点(拓扑冻结 / 包策略冻结 / import-publish 策略冻结 / scaffold 就绪)** + 3 份方向 RFC + 2-3 份代表 blueprint + absorption map 重写前置条件。
...
本阶段给 worker-matrix 留下(r2 narrower):

- NACP 1.4.0 shipped(narrower:wire-level 共享 shape 归位;BoundedEvalSink class 等 runtime 留原位)+ publishing pipeline skeleton 就绪(首发可 parallel 到 worker-matrix)
- 3 份**方向 RFC**(workspace RPC / remote compact / evidence forwarding)— 不含代码
- 1 份 absorption map + 2-3 份代表性 detailed blueprint(其他 7 份延到 worker-matrix P0 按需撰写)
- `workers/` 顶级目录 + 4 个 worker shell + **1 个 deployed URL(agent-core)+ 3 dry-run 证明脚手架同构** + per-worker CI
- 6 份 closure + handoff memo + worker-matrix charter r2 触发(state 从 deprecated flip 到 needs-rewrite-r2)
...
进入下一阶段(worker-matrix 实施 / r2 rewrite),我们将拥有(r2 narrower 净产出):

- `nacp-core 1.4.0` shipped(wire-level 共享 shape 已归位;BoundedEvalSink class 等 runtime 留原位)+ publishing pipeline skeleton 就绪(首发时机 owner 决定)
- **3 份方向 RFC**(workspace RPC / remote compact / evidence forwarding)— 不含代码,作为 worker-matrix 后续 phase input
- **1 份 absorption map + 2-3 份代表 detailed blueprint**;剩余 7 份由 worker-matrix P0 现场撰写
- `workers/` 顶级目录 + 4 个空 worker 项目 + **1 个 agent-core real deploy URL + 3 dry-run log** + per-worker CI matrix
- 6 份 closure + handoff memo
- `plan-worker-matrix.md` 触发进入 r2 rewrite

worker-matrix 阶段不会再问 **"workers/ 拓扑如何"、"包去哪里"、"import 从哪来"、"3 个 orphan 决策归属"** —— 这 4 类窄问题在本阶段已经被答完。worker-matrix 阶段只需做一件事:**按 map + 代表 blueprint 把 Tier B packages 吸收进 4 个 worker,装配 live agent turn loop;cross-worker split / 协议实装 / 全面发布切换 由 live loop 真实证据驱动**。
...
| 版本 | 日期 | 作者 | 修订综述 |
|---|---|---|---|
| **r1** | 2026-04-21 | Claude Opus 4.7 | 初版。定义 6-phase pre-worker-matrix 阶段:W0 NACP 吸收 / W1 跨 worker 新协议 / W2 GitHub Packages 发布 / W3 10 blueprint + dry-run / W4 4 workers 脚手架 + deploy / W5 closure。预估 4-5 周。 |
| **r2** | 2026-04-21 | Claude Opus 4.7 | Post-GPT-review narrowing。依据 `docs/plan-pre-worker-matrix-reviewed-by-GPT.md` 7 盲点裁决:<br/>• §0.5 新增 long-term vs first-wave 3 层分层原则<br/>• §1.5 publishing 改 parallel,非硬 blocker<br/>• §3 一句话目标重写,5 件工作均 narrower<br/>• §4.1 A(W0):BoundedEvalSink class 不搬 NACP;hooks catalog 拆 wire-shape vs runtime metadata<br/>• §4.1 B(W1):3 条新协议 **整体降级为 RFC-only**,不实装代码<br/>• §4.1 C(W2):skeleton 在 pre-phase,首发可 parallel<br/>• §4.1 D(W3):10 detailed → 1 map + 2-3 代表;dry-run 可选;若做选 capability-runtime 非 llm-wrapper<br/>• §4.1 E(W4):4 real deploy → 1 real(agent-core)+ 3 dry-run<br/>• §11 exit criteria:11 条 → 6 条(GPT §6 建议)<br/>• §11.4 明确从原 r1 降级的 exit items<br/>• 预估时长:4-5 周 → 2-3 周<br/>**净 scope 降幅 ~60%;RFC 文档化加强 ~80%** |

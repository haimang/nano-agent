...
- 已吸收 **DeepSeek R1**：`PP0-charter-truth-lock-action-plan.md` 顶部不再把 `00/01` design 放在“上游前序 / closure”语义里，而是改成“本计划锁定并校验的 cross-cutting / frontend contract design”，避免把 PP0 自己要法律化的设计误写成已经由上游 closure 生效的依赖。
- 已吸收 **DeepSeek R7 / R12 / Kimi R8 / Opus R12**：PP0 现在冻结了明确的 skeleton owner file `test/cross-e2e/16-pro-to-product-baseline-skeleton.test.mjs`，并把 PP0 baseline 场景写成当前代码已真实存在的 `PATCH /sessions/{id}/runtime → session.runtime.update → GET /sessions/{id}/runtime` 链路；PP1/PP2/PP3/PP4 只作为 `pending-PP*-implementation` 扩展点，不再含糊写成“未来也许覆盖任何 truth gate”。
- 已吸收 **DeepSeek R6 / R9 / R11**：PP0 统一补强了 closure 证据纪律，除了原有 `transport / trace_uuid / timestamps / verdict` 外，明确要求所有 phase closure 复用 `latency_alert.threshold_key / exceeded_count / accepted_by_owner / repro_condition`；并且在 PP0-PP6 的 §7.3 都补了同一套 amend 步骤，避免 design freeze 之后发生 silent drift。
- 已吸收 **Opus R6 / Kimi R7**：PP1 把 pending route 冻结为 `GET /sessions/{id}/confirmations?status=pending`，同时把 duplicate decision 的 wire 语义冻结为沿用 `jsonPolicyError()` 的 `facade-http-v1` envelope，而不是另起 bespoke 409 schema；若前端需要终态详情，动作是补读 detail/list route，而不是要求 decision 409 body 自带另一份 row mirror。
- 已吸收 **Kimi R2**：PP1 现在明确要求 closure 冻结共享 owner file 清单（至少 `workers/agent-core/src/host/do/session-do-runtime.ts`）并声明 extension point，PP3 只有在这个稳定声明出现后才能进入 shared file 改动窗口，避免只写“稳定后再改”这种不可执行的话术。
- 已吸收 **Opus R7 / DeepSeek R8 / Kimi R1**：PP2 不再用“possible shared helper”这种模糊表述，而是把 `workers/context-core/src/control-plane.ts` 的 `resolveBudget()` 冻结为 canonical budget owner；agent-core preflight 只允许复用同一预算语义，不允许发明第二套 truth。与此同时，PP2 还冻结了 prompt mutation 的 primary capture point：以 `runtime-mainline.ts` 在 `gateway.executeStream(exec)` 之前拿到的构造后 request 为准，必要时才在 `llm/gateway.ts` / `request-builder.ts` 暴露 test-only seam。
- 已吸收 **DeepSeek R8**：PP2 §7.2 现在明确要求实现前核对 `workers/orchestrator-core/migrations/` 当前最新编号仍为 `017`；若仓库已经出现 `018+`，必须先与 owner 确认下一可用编号，不能继续沿用旧基线口径。
- 已核实 **Kimi R6** 后决定**不新增 design 改动**：`03-context-budget-closure.md` 其实已经定义了 `protected fragment` 与最小实例（`<model_switch>` / `<state_snapshot>`）；本轮修订只把这些实例回填进 PP2 action-plan，防止执行者看 action-plan 时读不到 design 已经冻结的定义。
- 已吸收 **Opus R2 / DeepSeek R2**：PP3 已移除对 PP2 的硬依赖，且背景段改写为“persist 端已有 `buildWsHelperStorage() + helper.checkpoint()` 通道，但 checkpoint 内嵌 `replayFragment` 仍为 `null`、restore 端仍不对称”。这把“尚未开始”纠正为“半实现但关键断点仍在”，避免实施者误推翻已存在的 helper storage 通道。
- 已吸收 **Opus R2 / Kimi R2**：PP3 Phase 2 现在显式要求在实现期冻结 `replayFragment` 的去留决策（双写保留或 helperStorage-only），并把 PP1 shared-file 冲突的门槛写成可执行规则，而不是只说“注意冲突”。
- 已吸收 **DeepSeek R2 / R3 / Opus R1**：PP4 顶部依赖和 Phase 1 start gate 已改为只依赖 `PP1 closure`，不再把 `PP3 closure` 写成 charter 未授权的串行 prerequisite；同时保留“若 PP3 已闭合，可在 e2e 扩展 reconnect 场景”这种可选增强，而不是 start gate。
- 已吸收 **Opus R3 / Kimi R5**：PP4 现在冻结了 hook register 的 owner file 选择：建议新建 `workers/orchestrator-core/src/facade/routes/session-hooks.ts`，并把 `route-registry.ts` 与 `workers/agent-core/src/hooks/registry.ts` 一起写进 action-plan。职责也被写清：orchestrator facade 负责 auth / validation / session ownership，agent-core registry 负责存储 / 排序 / scope。
- 已吸收 **Opus R5 / R10**：PP4 不再把 `hook.broadcast` 写成抽象可见性，而是明确 generic `hook_emit` 只是 precedent，不能作为 PreToolUse closure 证据；若复用现有 `hook.broadcast` frame，则 payload 必须带 `caller: "pre-tool-use" | "step-emit"` 或等价 source 字段区分 provenance。同时把 redaction helper 的预期落点冻结到 `workers/agent-core/src/hooks/redaction.ts`，避免实现期写成分散的内联脱敏逻辑。
- 已吸收 **DeepSeek R4 / Opus R10 / Opus R5**：PP4 与 PP5 之间对 Q17 的责任边界已统一成一个法则：`PermissionRequest no handler` 在 PP4 dispatcher 层就是 terminal deny / diagnostics，PP5 policy chain 收到 `hook-no-handler` 后不再继续 fall through 到 HITL 或 allow。
- 已吸收 **DeepSeek R5 / Kimi R4 / Opus R11**：PP4 closure 现在也必须复用 PP0 的 latency alert 证据形态，PP4 的 DOD 文档项已经把 hook latency alert 写入完成定义，避免成为唯一不登记 latency 的 phase。
- 已吸收 **Opus R4 / Kimi R3 / Opus R8**：PP5 不再只写 `not-enforced/stored-only` 的自由文本，而是明确要求把 `not-enforced / stored-only / partial` 最终映射到 frozen 5-label 集 `live / first-wave / schema-live / registry-only / not-enforced`；同时，`unavailable` 三态不再被写成抽象“cross-package enum”，而是基于当前代码事实登记受影响 owner：`workers/orchestrator-core/src/entrypoint.ts`、`workers/agent-core/src/host/runtime-mainline.ts`、`workers/agent-core/src/host/env.ts`。
- 已核实 **Opus R8 / Kimi R3** 后做了**按代码事实的收窄**：当前 `authorizeToolUse` 的 decision union 还没有真正落到共享 package type，因此这轮没有强行虚构一个“现成 cross-package enum owner”；改为在 PP5 action-plan 中写出当前真实受影响文件清单，并要求若未来抽成 shared contract，再同步新的 owner file。
- 已吸收 **Opus R9 / O21**：PP6 现在把 hook docs placement 的 trigger 写实了——若 PP4 最终落成独立 `session-hooks.ts` facade owner，则新增 `clients/api-docs/hooks.md`；否则合并到现有专题并在 README 标唯一 owner。另一个细化是 debug routes：PP6 Phase 1 inventory 现在要求逐条标记 `frontend-inspector=yes/no`，避免到 final closure 才重新争论 worker-health/debug 是否算 frontend-facing。
- 已吸收 **DeepSeek R6 / Opus R6 / Opus R11**：PP6 的 final closure 不再只是“按 7 truth gates 给 verdict”，而是明确只消费 PP1-PP5 按 PP0 unified evidence shape 输出的证据，并复用统一的 latency alert 字段，这样 final closure 不会再次发明自己的证据格式。
- 已完成本轮修订的范围控制：本次实际改动集中在 `docs/action-plan/pro-to-product/PP0-PP6*.md` 与本文 §6；没有引入新的 design 文件、没有改写 review 正文 §0-§5、也没有为了迎合建议去虚构当前代码不存在的 shared contract / generated doc pipeline。

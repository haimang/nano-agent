# real-to-hero：真实客户端 API gap 深度调查（by Kimi）

> 本报告仅使用当前仓库真实代码与 `context/` 下三个外部 CLI 代码样本作为证据，不引用其他模型的分析结论。
> 目标：在最新 6-worker 架构下，判断 `orchestrator-core` 已提供的客户端接口距离“真实可运行的多租户 Agent-CLI / Web / 小程序客户端”还有多大 gap。

## 0. 结论先行

当前后端已具备一部分运行内核原材料：D1 有 identity / team / conversation / session / message / context snapshot / usage quota 表；`agent-core` 已能在有 `AI` binding 时走 Workers AI；`bash-core` 已有 WorkerEntrypoint RPC 与 authority 校验；`context-core` / `filesystem-core` 有大量库级能力。
...
| compound actions | `permission/decision`、`policy/permission_mode`（`workers/orchestrator-core/src/index.ts:233-242`） | permission HTTP mirror 已有，live WS gate 未真正落地 |

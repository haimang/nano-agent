...
| R2 | `[follow-up]` | `deferred` | Mini Program 的 WeChat code-level 登录入口已接线，但真实 appid / 微信开发者工具真机截图不在本仓自动化内。 | client hardening |
| R3 | `[follow-up]` | `deferred` | orchestrator timeline 不含 `llm.delta`，当前 live evidence 通过 D1 usage 证明 LLM mainline。若需要用户侧 token streaming，应在后续明确转存/relay 策略。 | Z5 / transport hardening |
...
Z4 的目标不是完整产品发布，而是让真实客户端与真实 runtime 进入同一条可观测链路。本轮已经完成 web + mini-program baseline、RPC authority preflight、remote D1 migration evidence、真实 Workers AI mainline smoke 与 quota evidence，因此 Z4 可收口为：**real-client baseline established / first real run evidence accepted / remaining work is client hardening and productization, not Z4 blocker**。

...
- **执行过的验证**：
  ...
  - `ls workers/orchestration-auth/` → **不存在**（与 Z1 一致；新建 worker）。
  - `find workers -name "migrations" -type d` → **0 命中**（所有 wave-A/B/C migrations 都是新建）。
  - `find workers -name "wrangler.jsonc"` → 5 个（agent/bash/context/filesystem/orchestrator-core 各一份，无 orchestration-auth）。
  ...
  - `ls test/package-e2e/orchestration-auth/` → **不存在**（与 Z1 [new] 标记一致）
...
- **F+5**：Z1 P1-01 已显式承担 `packages/orchestration-auth-contract/` typed contract package（这是我 Q1 Opus 第二意见的硬约束之一）——证明 owner 同意了的 Opus 推荐已被吸收。
...
- **F-4**：`find workers -name "migrations" -type d` → 0 命中。但 Z1 P1-02 / Z2 P1-01 / Z3 P4-01 都把 migrations 放在 `workers/orchestrator-core/migrations/`——而 ZX-D1 §3.2 / §7.2 F1 说 `所有身份写入权集中在 auth worker`（即 orchestration.auth）。**所有 migrations 在 orchestrator-core 而身份写入在 orchestration-auth**——deploy ordering 与 owner 责任链不一致。
...
- **F-7**：Z1 §1.4 directory tree（行 99）列出 `workers/orchestration-auth/wrangler.jsonc [new]`，但 Z1 P2-01 收口标准与 §5.2 都没有指明该 wrangler 的 D1 binding 是 `NANO_AGENT_DB`（同一个 D1 实例）还是独立 binding。Z1 P3-02 收口标准也只说 "refresh state 进入 nano_auth_sessions"——没说该表是哪个 worker 的 D1 binding 名。
...
- **事实依据**：
  ...
  - `ZX-d1 §7.2 F1`（行 244-251）：`Identity Core | 主要调用者 = orchestration.auth | 核心逻辑：所有身份写入权集中在 auth worker`。
...
- **为什么重要**：
  ...
  - 把 identity migrations 放在 orchestrator-core 意味着：(a) orchestrator-core 持有该 D1 的写权限（违反 design 的"identity write 权只在 auth worker"原则）；或 (b) orchestrator-core 仅用作 schema 容器但实际写由 orchestration-auth 做（这就要求 orchestration-auth 也 bind 同一个 D1，但 plan 没说）。
  - Deploy 时序：如果 orchestrator-core 在 orchestration-auth 之前 deploy，且 schema 在 orchestrator-core，那 schema 已经存在但无人能写；反之同理。
- **审查判断**：必须在 Z1 实施前明确 D1 ownership 模型。
- **建议修法**：
  - 推荐方案 A：所有 migrations 改放 `workers/orchestration-auth/migrations/`（auth worker 是身份写主），后续 wave-B/C 再分别归属（例 wave-B session migrations 留在 orchestrator-core，wave-C usage migrations 放在 agent-core 或单独的 quota worker）。
...
### R16. orchestration-auth wrangler binding 配置（D1 / 共享 secret / single-caller enforcement）未明确

- **严重级别**：`medium`
- **类型**：`security` + `delivery-gap`
- **事实依据**：
  - `Z1 §1.4 directory tree`（行 99-100）：`workers/orchestration-auth/wrangler.jsonc [new]`——文件标记 new，但内容未指认。
  - `Z1 §3 P2-01`（行 147）：工作内容 = "新建 workers/orchestration-auth/，接 NANO_AGENT_DB、PASSWORD_SALT、WECHAT_APPID、WECHAT_SECRET、JWT_SIGNING_KEY_<kid>"——绑定枚举 OK，但**未说"single-caller enforcement"如何实现**。
...
- **为什么重要**：
  - 如果 orchestration-auth 默认接受任何 service binding 调用，单 caller 规则名存实亡——任何 internal worker 都能调 mint endpoint。
...
- **建议修法**：
  - Z1 §4.2 P2-01 收口标准追加：
    ...
    2. 入口 `workers/orchestration-auth/src/index.ts` 在每个 RPC method 起始处调 `assertOrchestratorCaller(env)` helper（pattern 复用 `internal-policy.ts`）。
...
- **类型**：`docs-gap`
- **事实依据**：
  - `Z1 §4.1 P1-01`（行 164）：工作内容 = "新建 packages/orchestration-auth-contract/，定义 WorkerEntrypoint RPC input/output、shared auth envelope、typed error codes"——抽象。
...
| 编号 | 计划项 | 审查结论 | 说明 |
|------|--------|----------|------|
| Z1-S1 | `packages/orchestration-auth-contract/` typed contract | `partial` | 已纳入 in-scope；但接口枚举未列（R20） |
| Z1-S2 | 新建 `workers/orchestration-auth/` internal-only single-caller | `partial` | 创建动作明确；single-caller enforcement 配置未落实（R16） |
...
- **关闭前必须完成的 blocker**：
  ...
  4. **R4 修复**：决定 D1 migration owner——推荐方案 = identity migrations 移到 `workers/orchestration-auth/migrations/`，session/usage migrations 留在各自写主 worker；或全部放共享 `migrations/` 顶层目录。
...
- **可以后续跟进的 non-blocking follow-up**：
  ...
  4. **R14-R16**：Z0 扩 "specific implementation freeze" 职责 / Z5 charter exit criteria 提炼 / orchestration-auth single-caller enforcement 配置。

...
| F3 | index.ts 升级 | workers/bash-core/src/index.ts 从 probe 升为 capability entry(**binding-first, no public HTTP RPC surface**)| ✅ 暴露可被 `CAPABILITY_WORKER` service-binding `fetch(request)` 命中的 internal path:`/capability/call` + `/capability/cancel`(与 `remote-bindings.ts` 的 glue 完全对齐);保留 version-probe shape 兼容(至少 status/worker 字段);**不**对外暴露 `/tool.call.request` 或其他 public HTTP ingress |
...
- **核心逻辑**:
  - default export fetch handler,**binding-first**:
    - `GET /` or `/health` 返回 version-probe + `absorbed_runtime: true` 标志(保留 W4 外形兼容)
    - `POST /capability/call` → 路由到 `CapabilityRunner.execute`(body shape 为 `{requestId, capabilityName, body}`,与 `packages/session-do-runtime/src/remote-bindings.ts::callBindingJson('/capability/call', ...)` 约定 **byte-identical**)
    - `POST /capability/cancel` → 路由到对应 cancel 路径(body `{requestId, body}`)
  - `tool.call.*` 仍是 **NACP body schema**(来自 `@haimang/nacp-core`),**不是** bash-core 对外的 HTTP 路径命名;`/capability/call` 的 body `body` 字段承载 `tool.call.request`,`/capability/cancel` 的 body `body` 字段承载 `tool.call.cancel`
  - export `CapabilityRunner` / `FakeBashBridge` 等 public API
- **边界情况**:
  - D07 通过 `CAPABILITY_WORKER` service binding(非公开 HTTP)调用 bash-core;本 F3 提供的两条 internal path 就是 binding 被 `fetch(request)` 命中时的入口
  - 兼容 W4 probe JSON shape(不删字段)
  - **不**暴露任何 public HTTP RPC 表面;`/capability/*` 仅在 service-binding 内可达,外部 CDN/edge 路径不路由到这里
...
- [x] **Q2(已冻结 — 吸收 GPT R3)**:`workers/bash-core/src/index.ts` 的 `fetch` handler **只**通过 `CAPABILITY_WORKER` service binding 接入;**不**对外暴露 `/tool.call.request` 或其他 public HTTP ingress。service-binding 命中的 internal path 固定为 `/capability/call` + `/capability/cancel`(与 `remote-bindings.ts` glue 对齐);`tool.call.*` 仅作为 **body schema** 存在,不作为 bash-core 的 HTTP 路径命名
...
| v0.2 | 2026-04-23 | Claude Opus 4.7 | 吸收 D01-D09 GPT review R3:F3 收口为 binding-first / `/capability/call` + `/capability/cancel` internal path(与 `remote-bindings.ts` glue 对齐);`tool.call.*` 仅为 body schema,不作为 bash-core 公开 HTTP 路径命名 |

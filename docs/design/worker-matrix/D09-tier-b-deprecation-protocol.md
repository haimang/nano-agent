...
- **本次讨论的前置共识**:
  ...
  - 物理删除 = 下一阶段
  - CHANGELOG 每个 deprecated PR 同 commit 更新
...
| CHANGELOG entry | `CHANGELOG.md` 新增 `## [DEPRECATED] - YYYY-MM-DD` block |
...
- **[S3]** 定义 **CHANGELOG entry convention**:
  ...
  ```
- **[S4]** per-package deprecation PR(N = 9):每个 PR 只动 **1 个** `packages/<name>/README.md` + 该 package `CHANGELOG.md`
- **[S5]** deprecation 顺序建议:
...
| F2 | CHANGELOG entry convention | 统一 entry 格式 | ✅ 9 个 `CHANGELOG.md` `[DEPRECATED]` block 结构一致 |
...
#### F2: CHANGELOG entry convention

- **输入**:§5.1 S3
- **输出**:每 package 的 `CHANGELOG.md` 新 `[DEPRECATED]` block
- **核心逻辑**:时间 `YYYY-MM-DD` 取 PR merge 日;文字模板固定
- **边界情况**:若 package 没有 `CHANGELOG.md`,同 PR 内创建
- **一句话收口目标**:✅ **每 package 的 CHANGELOG 首条是 DEPRECATED entry**

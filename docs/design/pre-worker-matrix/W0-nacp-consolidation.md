...
| re-export | 原文件改为 `export * from "@haimang/nacp-core/..."` + deprecated JSDoc | 保证 1.3.0 消费者零破坏 |
...
| 每个 re-export 文件 | 原位置保留 3 个月起 | `export * from "@haimang/nacp-core/..."` + `@deprecated` JSDoc | 待 worker-matrix 完成 absorb 后,原位置删除;删除不在本阶段 |
...
`index.ts` 顶层 re-export 所有新 subdirectory 的 public symbols,保证消费者 `import { ... } from "@haimang/nacp-core"` 一次到位。
...
4. **取舍 4 — 新建子目录 `evidence/ hooks-catalog/ storage-law/`,不扁平化到 `src/` 根**
   ...
   - **接受的代价**:目录深度略增;import path 略长(`@haimang/nacp-core/evidence/sink` vs `@haimang/nacp-core/evidence-sink`)
   - **次要决策**:子目录内 `index.ts` 做 barrel export,外部 `import { EvalSinkEmitArgs, HookEventName } from "@haimang/nacp-core"` 等 shape symbols 仍可一步到位(BoundedEvalSink class 保留在 session-do-runtime,不从 nacp-core 再导出)
...
- **对 nano-agent 长期演进**:
  - 第三方实现者未来只需依赖 `@haimang/nacp-core`;不再需要拉 `session-do-runtime` / `workspace-context-artifacts` 等 validation-only 包
...
- **一句话收口目标**:✅ **Shape types 可从 `@haimang/nacp-core/evidence/sink-contract` import;BoundedEvalSink class 保留在 session-do-runtime 不动;所有 tests 保持 green**
...
- **输出**:
  ...
  - 原文件 `session-do-runtime/src/cross-seam.ts` 头部改为 `export { CrossSeamAnchor, CROSS_SEAM_HEADERS, buildCrossSeamHeaders, readCrossSeamHeaders } from "@haimang/nacp-core"`;保留后续 165 行(failure taxonomy + startup queue)作为 agent.core-specific content
...
- **测试矩阵**:
  - `pnpm --filter @haimang/nacp-core test` — 247+ 保持 + 新增 C1-C5 的 unit test smoke
...
- [ ] **Q2**:re-export wrapper 的 `@deprecated` JSDoc 指向文本(建议:"`Moved to @haimang/nacp-core as part of 1.4.0 consolidation. Will be removed in worker-matrix phase.`")

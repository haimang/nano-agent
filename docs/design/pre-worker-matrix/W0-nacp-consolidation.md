...
### 0.4 代码事实核查产生的 3 处 charter 修正

核查原码后,发现 charter §7.1 内部描述与当前代码不完全对齐:

| charter 原文 | 代码事实 | 本 design 修正 |
|---|---|---|
| "hook catalog 8 个 v1 event" | 实际已是 **18 events**(B5 expansion shipped:Class A 8 + Class B 4 + Class D 6) | W0 吸收**全部 18 events**的 vocabulary,而非"8 个 v1" |
...
| Storage law helpers + `_platform` const | `packages/storage-topology/src/keys.ts` (85) + `refs.ts` (166) | 251 合计 | `buildDoStorageRef / buildR2Ref / buildKvRef / KV_KEYS / parseTenantKey / _PLATFORM_RESERVED` 等 |
...
├── observability/                    (现有)
├── evidence/                         【W0 新建目录】
│   ├── sink.ts                       【W0 新建】— BoundedEvalSink + types + extractMessageUuid
│   └── vocabulary.ts                 【W0 新建】— 4 kinds schema
├── hooks-catalog/                    【W0 新建目录】
│   └── index.ts                      【W0 新建】— HookEventName + HookEventMeta + 18-event registry
└── storage-law/                      【W0 新建目录】
    ├── platform.ts                   【W0 新建】— _PLATFORM_RESERVED + feature-flags 例外常量
...
- **[S1]** `BoundedEvalSink` + 附属类型搬到 `nacp-core/src/evidence/sink.ts`;`session-do-runtime/src/eval-sink.ts` re-export
...
- **[S4]** `HookEventName / HookEventMeta / HOOK_EVENT_META_REGISTRY` 搬到 `nacp-core/src/hooks-catalog/index.ts`;`hooks/src/catalog.ts` re-export;dispatch 逻辑保留
- **[S5]** `buildDoStorageRef / buildR2Ref / buildKvRef / KV_KEYS / parseTenantKey / _PLATFORM_RESERVED` 搬到 `nacp-core/src/storage-law/{builders.ts,platform.ts}`;`storage-topology/src/{keys,refs}.ts` re-export
...
4. **取舍 4 — 新建子目录 `evidence/ hooks-catalog/ storage-law/`,不扁平化到 `src/` 根**
   ...
   - **次要决策**:子目录内 `index.ts` 做 barrel export,外部 `import { BoundedEvalSink } from "@nano-agent/nacp-core"` 仍可一步到位
...
| `storage-law` 吸收与 `tenancy/` 已有 helper 产生名字冲突 | symbol 重名 | import ambiguity | §5.3 边界清单已确认 tenancy 与 storage-law 是两个关注点;符号命名不同(`verifyTenantBoundary` vs `_PLATFORM_RESERVED`) |
...
| 编号 | 功能名 | 描述 | 一句话收口目标 |
|---|---|---|---|
| C1 | Evidence sink 吸收 | BoundedEvalSink + 4 附属类型 + extractMessageUuid → `nacp-core/src/evidence/sink.ts` | ✅ 原位置 re-export;所有 sink consumer 零 break |
...
| C4 | Hooks catalog vocabulary 吸收 | 18 events + HookEventMeta → `nacp-core/src/hooks-catalog/index.ts` | ✅ 18 events metadata 完整 re-export;dispatch runtime 留 hooks 包 |
...
- **输出**:
  - `packages/nacp-core/src/evidence/sink.ts`(292 行,byte-identical semantics)
  - `packages/session-do-runtime/src/eval-sink.ts` → 缩成 re-export(~15 行 + `@deprecated` JSDoc)
- **主要调用者**:`NanoSessionDO` 构造函数(256-280 行)
- **核心操作**:
  1. 复制 `session-do-runtime/src/eval-sink.ts` 的全部 export 到 `nacp-core/src/evidence/sink.ts`
  2. 调整 nacp-core 内 import(若有,如 `../types.js` 等)
  3. 原文件改为 `export { BoundedEvalSink, ... } from "@nano-agent/nacp-core"` + `@deprecated` 指向新位置
  4. test 迁移:`packages/nacp-core/test/evidence/sink.test.ts` 新增(复制 session-do-runtime 的对应 test);原 test smoke 保留
- **边界情况**:
  - nacp-core build 不能有循环引用(若 sink 依赖 `envelope.ts` 的 `NacpEnvelope` 类型,OK;反向则禁止)
  - re-export 文件保留运行时 import side effects(应该没有,sink 是 pure class)
- **一句话收口目标**:✅ **`import { BoundedEvalSink } from "@nano-agent/session-do-runtime"` 与 `import { BoundedEvalSink } from "@nano-agent/nacp-core"` 均返回同一 class;所有 session-do-runtime tests 保持 green**
...
#### C4: Hooks catalog vocabulary 吸收

- **输入**:`packages/hooks/src/catalog.ts`(285 行,含 18 events metadata)
- **输出**:
  - `packages/nacp-core/src/hooks-catalog/index.ts` 新建(~285 行)含:
    - `HookEventName` enum/string-literal-union(18 values)
    - `HookEventMeta` interface(blocking / allowedOutcomes / payloadSchema / redactionHints)
    - `HOOK_EVENT_META_REGISTRY` 或等价 record(18 entries)
  - `hooks/src/catalog.ts` 改为 re-export `export { HookEventName, HookEventMeta, HOOK_EVENT_META_REGISTRY } from "@nano-agent/nacp-core"`
- **charter 对齐修正**:charter §7.1 原称 "8 个 v1 event" — 实际 B5 shipped 18 events(Class A 8 + Class B 4 + Class D 6);W0 按 18 个吸收(§6.1 取舍 5)
- **核心操作**:
  - hooks 包内 dispatch runtime(emit / outcome reducer / session mapper)保留不迁
  - catalog vocabulary 纯数据 — 搬到 nacp-core 后 hooks 包从 re-export 消费
- **边界情况**:
  - hooks 包 dispatch 仍 import `HookEventMeta` 走 nacp-core path — 形成 hooks → nacp-core 依赖(原本就有,因为 hooks 已经依赖 nacp-core for envelope)
- **一句话收口目标**:✅ **18 events 全部搬到 nacp-core;hooks dispatch runtime 无感 re-export 继续工作;Class A-B-D 覆盖完整**

#### C5: Storage law 吸收

- **输入**:`packages/storage-topology/src/keys.ts`(85)+ `refs.ts`(166)
- **输出**:
  - `packages/nacp-core/src/storage-law/platform.ts` 新建(`_PLATFORM_RESERVED` 常量 + `KV_KEYS.featureFlags()` 等 ~40 行)
  - `packages/nacp-core/src/storage-law/builders.ts` 新建(`buildDoStorageRef / buildR2Ref / buildKvRef / parseTenantKey` 等 helpers ~120 行)
...
- **一句话收口目标**:✅ **builders + `_PLATFORM_RESERVED` 在 nacp-core;adapters 仍在 storage-topology;所有 ref 仍过 NacpRefSchema**
...
- **nacp-core CHANGELOG 1.4.0 entry 草案**:
  ...
  ### Added
  - `evidence/sink.ts` — BoundedEvalSink + EvalSink* types + extractMessageUuid (absorbed from @nano-agent/session-do-runtime)
  ...
  - `hooks-catalog/` — HookEventName (18 values) + HookEventMeta + HOOK_EVENT_META_REGISTRY (absorbed from @nano-agent/hooks; runtime dispatch remains in hooks package)
  - `storage-law/` — buildDoStorageRef/buildR2Ref/buildKvRef/parseTenantKey helpers + `_PLATFORM_RESERVED` constants + `KV_KEYS.featureFlags()` exception
...
- **测试矩阵**:
  ...
  - `test/b7-round2-integrated-contract.test.mjs` — 5/5 保持(BoundedEvalSink 契约是 B7 关键依赖)
...
| 文件:行 | 内容 | 借鉴点 |
|---|---|---|
| `packages/session-do-runtime/src/eval-sink.ts:40-77` | BoundedEvalSink types | 直接复制到 nacp-core/evidence/sink.ts |
...
| `packages/storage-topology/src/keys.ts:38-64` | `_platform/` exception + KV_KEYS | 直接 copy 到 nacp-core/storage-law/platform.ts |

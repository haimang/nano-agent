...
- **本次讨论的前置共识**：
  ...
  - 当前真实起点是 `workers/orchestrator-core/src/user-do.ts` 仍以 registry/relay + fetch-backed internal control-plane 为主，DO SQLite/Alarm hot-state 仍需从零 bringup。
  - `packages/nacp-session/src/{messages,heartbeat,replay,frame}.ts` 已提供现成 session primitives；Z2/Z4 负责消费，而不是重新发明。
- **显式排除的讨论范围**：
...
- **判定方法**：
  1. session 结束后，`nano_conversation_sessions / nano_conversation_turns / nano_conversation_messages` 仍可直接支持 history/timeline 查询。
  2. 同一 trace fixture 走 HTTP/WS 交互后，public history API 与 D1 直接查询结果结构一致。
  3. `conversation_uuid` 与 `session_uuid` 在 read path 中不再混用。

#### F2: `Context/Audit Truth`

- **输入**：turn lifecycle、context assembly 结果、runtime/auth/quota side-effects
- **输出**：`nano_conversation_context_snapshots` 与 `nano_session_activity_logs`
- **主要调用者**：`orchestration.core`、后续 audit/history 查询
- **核心逻辑**：把每轮上下文快照与 append-only activity log 纳入 shared truth，并统一复用 redaction / trace linkage 纪律。
- **边界情况**：
  - activity log payload 走 write-side redaction；不允许直接裸写
  - context snapshot 只保留 first-wave 必需字段，不抢做 `context_items` richness
- **一句话收口目标**：✅ **context 与 audit 不再只能靠热态或 scattered logs 回想**
- **判定方法**：
  1. `nano_conversation_context_snapshots` 能按 `conversation_uuid / turn_uuid` 回看最近一次上下文快照。
  2. `nano_session_activity_logs` 至少能按 `team_uuid / trace_uuid / session_uuid` 查询 auth、runtime、quota 关键事件。
  3. activity payload 中不存在未经 redaction 的 token / secret / raw authority 泄漏。
...
- **核心逻辑**：只保留 real client 必需热路径，不抢 D1 的 SSOT 地位；hot-state 冻结为四组最小集合：`conversation_index`、`active_pointers`、`recent_frames/reconnect_hints`、`short-lived caches`。
- **边界情况**：
  - `conversation_index <= 200 / user`
  - `active_pointers` 每 user 最多 1 条 active conversation + 1 条 active session
  - `recent_frames <= 50 / session`，reconnect 超出窗口部分必须回落 D1
  - DO Alarm 每 `10 min` 负责 trim recent frames、过期 reconnect hints、evict short-lived cache
  - 所有热态必须能从 D1 重新构建
- **一句话收口目标**：✅ **user DO 已从 registry/relay owner 升格为最小 user-state host**
- **判定方法**：
  1. 清空 DO hot-state 后，history / reconnect 仍能从 D1 + replay cursor 恢复最近窗口。
  2. Alarm 跑过后 recent_frames / caches 不会无限膨胀。
  3. Z2 review 不再把 user DO 判定为“只有 relay 没有真实 stateful uplift”。

#### F4: `RPC Kickoff`

- **输入**：当前 `/internal/sessions/*` fetch-backed reality、typed auth/trace/envelope
- **输出**：control-plane 第一条 dual-implemented RPC path
- **主要调用者**：`orchestration.core`、`agent.core`
- **核心逻辑**：Z2 内部允许先用 `status` 做 RPC scaffold smoke，但 closure proof 必须落在 `start`：同一组 fixtures 跑 fetch path 与 RPC path，比较返回 envelope、authority/trace stamp、以及 D1 row diff。
- **边界情况**：
  - first-wave client/session messages 固定消费 `session.start`、`session.followup_input`、`session.cancel`、`session.stream.ack`、`session.heartbeat`
  - `session.resume` / replay hardening 的完整 client 体验收口留给 Z4，但 Z2 必须保留 `frame / replay / heartbeat` compatibility
- **一句话收口目标**：✅ **control-plane RPC 已从抽象方向变成主路径证明**
- **判定方法**：
  1. `status` RPC smoke 可证明 WorkerEntrypoint / typed contract / envelope legality 全链路成立。
  2. `start` dual-impl parity 测试通过：返回 envelope deep-equal，D1 row diff = empty，authority/trace stamp 一致。
  3. Z2 closure 时新增控制面不再走新的 fetch-backed HTTP 面。
...
- **测试覆盖要求**：history、reconnect、timeline、conversation list、首条 RPC parity 都要有证明。
  - **测试基础设施基线**：沿用现有 package-e2e / cross-e2e harness；Z2 只新增 session/history/reconnect/RPC parity 证明，不新建平行测试体系。
...
- [ ] **已冻结答案需在实施中消费**：Q5-Q7 已在 `ZX-qna.md` 回填，Z2 action-plan 直接采用 activity log 字段、DO hot-state 容量/Alarm、`start` parity proof。
- [ ] **关联 Issue / PR**：conversation truth、context/audit truth、stateful uplift、history API、RPC kickoff。
- [ ] **实施前需要继续对齐的 cross-cutting 文档**：
...
- [x] **Q5**：activity log = 单 append-only 表 + 必要 views/query helpers。
- [x] **Q6**：DO hot-state = `conversation_index + active_pointers + reconnect/timeline hints + short-lived caches` 四组。

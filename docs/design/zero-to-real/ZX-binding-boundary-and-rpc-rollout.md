...
- **本次讨论的前置共识**：
  ...
  - 当前真实 worker 拓扑是 **5 worker reality**：`orchestrator-core / agent-core / bash-core / context-core / filesystem-core`；其中 `context-core / filesystem-core` 仍更接近 probe-only library shells，而非已 load-bearing 的 internal mesh worker。
  - `orchestrator.auth` 是 Z1 明确要创建的第 6 个 worker，不是既有 reality。
- **显式排除的讨论范围**：
...
| pure internal transport | 只接受内部 caller 的 transport 形态 | `orchestrator.auth` day-1 必须如此 |
...
|------------|----------|---------------------|------|
| Auth foundation | boundary -> auth | 强 | `orchestrator.auth` 是否 internal-only 由本设计冻结 |
...
| `orchestrator.auth` public route | 常见 auth service 暴露方式 | 与“唯一对外入口”直接冲突 | 否 |
...
- **[S2]** 冻结 `orchestrator.auth` internal-only / only called by `orchestration.core`。
...
| 项目 | 判定 | 理由 |
|------|------|------|
| `orchestrator.auth` 是否可有 public `/auth/*` | out-of-scope | public auth 只能由 `orchestration.core` 代理 |
...
3. **取舍 3**：我们选择 **`orchestrator.auth` single-caller** 而不是 **auth worker 被多个内部 worker共享调用**
...
- **核心逻辑**：
  ...
  2. `orchestrator.auth` 只接受 `orchestration.core`
  3. `agent.core` 只面向 internal runtime mesh
  4. `bash/context/filesystem` 只由 `agent.core` 消费
- **边界情况**：
  - probe 可继续存在
  - public business route 不可外溢
  - `context.core` / `filesystem.core` 在 zero-to-real 里默认仍按 library-shell / probe-only posture 处理，不强行写成 load-bearing network mesh
- **一句话收口目标**：✅ **worker 间边界已经先于代码实现被冻结**
- **判定方法**：
  1. 文档明确区分 current topology（5 worker reality）与 target topology（新增 `orchestrator.auth` 后的 6 worker）。
  2. `orchestration.core` 继续作为唯一 public façade；其他 worker 只保留 probe / internal surface。
  3. `agent.core` 仍是 runtime mesh head，不发生 façade 直连 `context/filesystem` 的 boundary drift。

**current vs target topology（frozen baseline）**

| 角色 | current reality | zero-to-real target |
|------|-----------------|---------------------|
| `orchestration.core` | public façade + user DO + fetch-backed control-plane relay | 唯一 public façade + auth/session truth owner |
| `orchestrator.auth` | 不存在 | Z1 新建 internal-only auth worker，仅接受 `orchestration.core` 调用 |
| `agent.core` | internal runtime head + legacy `/internal/*` owner | internal runtime head + control-plane RPC consumer/provider |
| `bash.core` | live capability worker | 继续由 `agent.core` 单向消费 |
| `context.core` | probe-only / library-shell posture | Z 阶段默认维持该 posture，不抢跑成独立业务 mesh |
| `filesystem.core` | probe-only / library-shell posture | Z 阶段默认维持该 posture，不抢跑成独立业务 mesh |

#### F2: `Auth Internalization`

- **输入**：public `/auth/*` façade、internal auth calls、JWT/identity secrets
- **输出**：single-caller auth boundary
- **主要调用者**：`orchestration.core`
- **核心逻辑**：Q1 已冻结 `orchestrator.auth = WorkerEntrypoint RPC-first + typed contract`；Z1 必须把 auth mint/verify/write truth 从 runtime mesh 中抽离出来，不允许 `agent.core` / `bash.core` 直连 auth truth。
- **边界情况**：
  - 仅当 Cloudflare 平台限制真实阻塞 RPC 时允许 fetch-binding shim
  - 该 shim 若出现，最晚在 Z2 closure 前 retire
- **一句话收口目标**：✅ **JWT/identity 能力只在 auth boundary 内生效，不在 runtime mesh 漫流**
- **判定方法**：
  1. `workers/orchestrator-auth/` 已存在且仅接受 `orchestration.core` 调用。
  2. `agent.core / bash.core / context.core / filesystem.core` 不直接持有 auth write 或 mint surface。
  3. single-caller negative tests 能证明其他 caller 被 typed reject。

#### F3: `Control-Plane RPC Kickoff`
...
- **核心逻辑**：
  ...
  - 允许先用 `status` 做 scaffold smoke，但 closure proof 必须把 `start` 做成首条 dual-implemented 方法
...
- **判定方法**：
  1. `status` RPC smoke 打通 WorkerEntrypoint + typed contract + NACP envelope precheck。
  2. `start` dual-impl parity 测试通过，返回 envelope / authority / trace / D1 row diff 全部一致。
  3. Z2 之后不再新增 fetch-backed control-plane HTTP 面。

**RPC signature 草案（first-wave）**

```ts
export interface StartSessionRpcInput {
  authority: NacpAuthority;
  trace: NacpTrace;
  body: SessionStartBody;
}

export interface StartSessionRpcOutput {
  session_uuid: string;
...
#### F4: `Stream Residual Governance`

- **输入**：Z2/Z3 之后仍存在的 stream relay / reconnect / pull seams
- **输出**：受控 residual inventory
- **主要调用者**：Z4 client hardening、closure / handoff 作者
- **核心逻辑**：stream-plane 允许过渡，但必须被明示为 residual seam；任何保留项都要给出 owner、原因、退役候选阶段，不允许继续“默认存在、没人认领”。
- **边界情况**：
  - residual seam 可以存在，但不能借机扩张为新的 public/control-plane contract
  - stream-plane hardening 继续优先消费 `nacp-session` 已有 frame/heartbeat/replay primitives
- **一句话收口目标**：✅ **stream residual 已被治理成显式 backlog，而不是隐性长期债务**
- **判定方法**：
  1. residual inventory 至少包含 seam 名称、owner、原因、风险、退役候选阶段。
  2. stream residual 数量不高于 zero-to-real 起始基线。
  3. code review 能用 grep / inventory 表证明“没有新增 control-plane HTTP”。
...
- [ ] **已冻结答案需在实施中消费**：Q1 / Q4 / Q7 / Q10 已在 `ZX-qna.md` 回填，ZX-binding 后续只负责把这些答案落成拓扑与迁移顺序。
...
- [x] **Q1**：`orchestrator.auth` exact transport = WorkerEntrypoint RPC-first。
- [x] **Q7**：Z2 closure proof 选 `start` dual-impl；允许先用 `status` 做 scaffold smoke。
- [x] **Q10**：Mini Program first-wave baseline = HTTP in + WS out，stream hardening 后置收口。

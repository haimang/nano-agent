...
- **本次讨论的前置共识**：
  ...
  - `orchestration.auth` 是 Z1 明确要创建的第 6 个 worker，不是既有 reality。
- **显式排除的讨论范围**：
...
- **判定方法**：
  1. 文档明确区分 current topology（5 worker reality）与 target topology（新增 `orchestration.auth` 后的 6 worker）。
...
| `orchestration.auth` | 不存在 | Z1 新建 internal-only auth worker，仅接受 `orchestration.core` 调用 |
...
- **核心逻辑**：Q1 已冻结 `orchestration.auth = WorkerEntrypoint RPC-first + typed contract`；Z1 必须把 auth mint/verify/write truth 从 runtime mesh 中抽离出来，不允许 `agent.core` / `bash.core` 直连 auth truth。
- **边界情况**：
...
- **判定方法**：
  1. `workers/orchestration-auth/` 已存在且仅接受 `orchestration.core` 调用。
...
- [x] **Q1**：`orchestration.auth` exact transport = WorkerEntrypoint RPC-first。

...
- **本次讨论的前置共识**：
  ...
  - 当前真实起点是 `workers/agent-core/src/llm/gateway.ts` 仍为 stub，`beforeCapabilityExecute()` 只覆盖 tool/capability path，LLM path 尚无对称 gate。
- **显式排除的讨论范围**：
  - full fallback chain
...
- **判定方法**：
  1. `agent.core` 通过 `AI` binding 与 frozen first-wave model 产生真实流式输出。
  2. fake provider 只保留在 test/demo path，不再是默认 deploy path。
  3. `nano_usage_events.provider_key='workers-ai'` 的写入可被 trace/session 关联回看。
...
- **一句话收口目标**：✅ **配额不是日志，而是门禁**
- **判定方法**：
  1. tool gate 复用 `workers/bash-core/src/executor.ts::beforeCapabilityExecute()`。
  2. LLM gate 在 `workers/agent-core/src/kernel/runner.ts` 的 invoke path 前新增对称 hook（如 `beforeLlmInvoke`）。
  3. deny 会同时产生 typed user-visible error 与 `nano_session_activity_logs.event_kind='quota.deny'` 记录。

#### F3: `Usage/Balance Persistence`

- **输入**：每次 llm/tool allow/deny verdict、estimated/requested cost、实际 usage
- **输出**：`nano_usage_events` append-only 记录与 `nano_quota_balances` 最新视图
- **主要调用者**：quota authorizer、`agent.core`
- **核心逻辑**：在 allow/deny 之后写 usage event，并以幂等 key 更新 team 级 balance；tool 与 LLM 走同一套 `team_uuid + trace_uuid + session_uuid` 对齐面。
- **边界情况**：
  - deny 事件也要落 usage/audit，不能只写 allow
  - 第一版只冻结最小 remaining/limit state，不引入 full ledger / alerts plane
- **一句话收口目标**：✅ **配额决策与资源消耗已具备持久化真相**
- **判定方法**：
  1. allow/deny fixtures 都会写入 `nano_usage_events`，并带 `idempotency_key` 防重复记账。
  2. `nano_quota_balances` 可从 team 维度读出 remaining/limit 视图。
  3. 相同 trace 重放不会重复扣减 balance。

#### F4: `Runtime Evidence`

- **输入**：llm invoke、tool invoke、quota verdict、provider failure
- **输出**：trace-linked runtime evidence + activity log readback
- **主要调用者**：`agent.core`、`bash.core`、后续 Z4 client evidence pack
- **核心逻辑**：把 runtime 执行中的 provider/model、tool name、verdict、resource usage、error code 全部回挂到 `trace_uuid + session_uuid + team_uuid`。
- **边界情况**：
  - provider-native raw stream 不直透客户端
  - evidence 必须走统一 mapper，而不是 scattered log statements
- **一句话收口目标**：✅ **真实运行不再是黑盒，而是可回放、可追责的执行链**
- **判定方法**：
  1. 任一真实 run 都能从 trace/activity/usage 三侧拼出同一条执行时间线。
  2. provider error 与 quota deny 都能在 client-visible stream 与 audit/readback 中被同时观察到。
  3. Z4 evidence pack 不需要额外猜测 provider/tool 细节即可重建 runtime verdict。
...
- **测试覆盖要求**：Workers AI happy path、quota allow/deny、usage/balance 写入、evidence linkage 都需证明。
  - **测试基础设施基线**：继续复用 package-e2e / cross-e2e harness，并在 runtime proof 中增加 Workers AI smoke、quota allow/deny、usage/evidence integration fixtures。
...
- [ ] **已冻结答案需在实施中消费**：Q8-Q9 已在 `ZX-qna.md` 回填，Z3 action-plan 直接采用 Workers AI required / DeepSeek skeleton、LLM+tool 双 gate。
- [ ] **关联 Issue / PR**：Workers AI mainline、runtime quota gate、usage tables、runtime evidence。
...
- [x] **Q8**：Workers AI 是唯一 required provider，DeepSeek 只保留 optional skeleton / fallback track。
- [x] **Q9**：quota deny 覆盖 llm + tool 两类真实消耗路径。

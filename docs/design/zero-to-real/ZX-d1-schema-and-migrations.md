...
- **判定方法**：
  1. `orchestration.auth` 是 identity core 唯一主写者。
...
- **主要调用者**：`orchestration.core`、`agent.core`、`orchestration.auth`
...
| 表名 | 最小字段冻结 | 主写 worker |
|------|--------------|-------------|
| `nano_users` | `user_uuid, created_at` | `orchestration.auth` |
| `nano_user_profiles` | `user_uuid, display_name, avatar_url, updated_at` | `orchestration.auth` |
| `nano_user_identities` | `identity_uuid, user_uuid, identity_provider, provider_subject, provider_subject_normalized, team_uuid, created_at, last_login_at` | `orchestration.auth` |
| `nano_teams` | `team_uuid, owner_user_uuid, created_at, plan_level` | `orchestration.auth` |
| `nano_team_memberships` | `membership_uuid, team_uuid, user_uuid, membership_level, created_at` | `orchestration.auth` |
| `nano_auth_sessions` | `auth_session_uuid, user_uuid, team_uuid, refresh_token_hash, expires_at, rotated_from_uuid, created_at, revoked_at` | `orchestration.auth` |
| `nano_team_api_keys` | `api_key_uuid, team_uuid, key_hash, label, created_at, revoked_at` | `orchestration.auth` |
| `nano_conversations` | `conversation_uuid, team_uuid, created_by_user_uuid, title, created_at, archived_at` | `orchestration.core` |
| `nano_conversation_sessions` | `session_uuid, conversation_uuid, team_uuid, status, started_at, ended_at, trace_uuid` | `orchestration.core` |
| `nano_conversation_turns` | `turn_uuid, session_uuid, conversation_uuid, team_uuid, role, status, started_at, ended_at, trace_uuid` | `agent.core` |
| `nano_conversation_messages` | `message_uuid, turn_uuid, session_uuid, conversation_uuid, team_uuid, message_kind, body_json, created_at, trace_uuid` | `agent.core` |
| `nano_conversation_context_snapshots` | `snapshot_uuid, turn_uuid, session_uuid, conversation_uuid, team_uuid, snapshot_kind, summary_ref, prompt_token_estimate, created_at, trace_uuid` | `agent.core` |
| `nano_session_activity_logs` | `activity_uuid, team_uuid, actor_user_uuid, conversation_uuid, session_uuid, turn_uuid, trace_uuid, event_seq, event_kind, severity, payload, created_at` | shared append-only by owner event source |
...
- `nano_conversation_turns(session_uuid, started_at)`
...
| worker | 允许主写的表组 |
|--------|----------------|
| `orchestration.auth` | identity core, auth sessions, team api keys |
| `orchestration.core` | conversations, conversation sessions |
| `agent.core` | turns, messages, context snapshots, usage events, quota balances |
| `shared append-only emitters` | `nano_session_activity_logs` 仅允许 append，不允许跨 worker update/delete |

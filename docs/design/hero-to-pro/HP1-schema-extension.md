...
> - `context/gemini-cli/packages/core/src/services/chatRecordingTypes.ts:92-140`
> - `context/gemini-cli/packages/core/src/services/chatRecordingService.ts:303-470,510-532,799-818`
> - `context/gemini-cli/packages/core/src/utils/checkpointUtils.ts:15-24,84-157`
> - `context/gemini-cli/packages/cli/src/ui/commands/restoreCommand.ts:43-123`
> 关联 QNA / 决策登记:
> - `docs/design/hero-to-pro/HPX-qna.md`（待所有 hero-to-pro 设计文件落地后统一汇总；本设计先冻结 schema 级结论）
> 文档状态: `reviewed`
...
- **主要调用者**：HP5、HP6、HP9
- **核心逻辑**：新增 `nano_session_todos`、`nano_session_temp_files`、`nano_session_confirmations`；给 `nano_session_files` 增 provenance columns
- **边界情况**：
  - temp file 与 artifact 不能混表
  - confirmation kinds 要为 future 扩展留枚举空间
...
- **核心逻辑**：新增 `nano_session_checkpoints`、`nano_checkpoint_file_snapshots`、`nano_checkpoint_restore_jobs`、`nano_workspace_cleanup_jobs`
- **边界情况**：
...
| 文件:行 | 内容 | 借鉴点 | 备注 |
|---------|------|--------|------|
| `N/A` | 本批次没有直接采用 mini-agent 源码作为 schema precedent | HP1 主要依赖当前仓库 D1 baseline 与 `context/` 源码 precedent | 不再通过二手 study 转述 |
...
|----------------|------|----------|----------|------|----------|
| `HP1-D1` | hero-to-pro 新 migration 是否固定为 `007-013` 七个文件？ | HP1 / HP9 | 是，按主题拆分；不再保留旧 `008/009-016` 口径 | `frozen` | 当前 repo baseline 仅到 `006`，且 durable truth 缺口已分散落在 `workers/orchestrator-core/migrations/002-session-truth-and-audit.sql:7-169`, `003-usage-quota-and-models.sql:17-89,56-129`, `004-session-files.sql:6-27`, `006-error-and-audit-log.sql:22-103` |
| `HP1-D2` | checkpoint / file snapshot / restore job 是否必须在 HP1 一次性落表？ | HP1 / HP4 / HP7 | 必须，否则后续 phase 会破戒 | `frozen` | 现状只有 context snapshot 与 artifact file：`workers/orchestrator-core/migrations/002-session-truth-and-audit.sql:86-105`, `004-session-files.sql:6-27`；而 Gemini precedent 已把 checkpoint / restore 做成产品真相：`context/gemini-cli/packages/core/src/utils/checkpointUtils.ts:15-24,84-157`, `context/gemini-cli/packages/cli/src/ui/commands/restoreCommand.ts:43-123`, `context/gemini-cli/packages/core/src/commands/restore.ts:11-57` |
| `HP1-D3` | 若 HP3-HP7 发现 schema blocker，是否允许直接新增 migration？ | HP1 / HP3-HP10 | 不允许；必须走 `HP1 schema correction` | `frozen` | 当前 schema 断点已能从 baseline 直接看到：`workers/orchestrator-core/migrations/002-session-truth-and-audit.sql:41-62`, `003-usage-quota-and-models.sql:56-86`, `004-session-files.sql:6-27`；因此 HP1 必须先一次性补齐 durable footing，而不是把补 migration 正常化 |

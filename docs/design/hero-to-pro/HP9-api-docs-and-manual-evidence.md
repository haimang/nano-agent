...
- **核心逻辑**：
  ...
  - 这里显式修正 charter 文案里曾出现过的“新增 6 份”笔误：HP9 以 `现有 11 + 新增 7 = 最终 18` 为 authoritative count，README 属于保留包的一部分，不单独再算“新增”
...
|----------------|------|----------|----------|------|----------|
| `HP9-D1` | api docs 应按 worker 模块切还是按产品 surface 切？ | HP9 / clients | 按产品 surface 切 | `pending-HPX-qna` | 当前 `session.md` 已因混装 models/context/files 而过载，说明 phase 成熟后必须拆开：`clients/api-docs/session.md:1-27` |
| `HP9-D2` | manual evidence 是否仍允许继续 defer？ | HP9 / HP10 | 否；hard gate | `pending-HPX-qna` | zero-to-real final closure 已把 manual evidence 列为明确遗留 gap，HP9 不应重演：`docs/issue/zero-to-real/zero-to-real-final-closure.md:99-105` |
| `HP9-D3` | prod schema baseline 是否可只依赖仓内 migrations？ | HP9 / HP10 | 否；必须 remote/prod 校对 | `pending-HPX-qna` | `workers/orchestrator-core/wrangler.jsonc:33-41,90-97` |
| `HP9-D4` | 18 份 docs 是否全部走同样强度 review？ | HP9 / reviewers | 否；rewrite/new 深审，稳定 docs sanity check | `pending-HPX-qna` | 当前文档成熟度明显不均：`clients/api-docs/error-index.md:1-12`, `clients/api-docs/session.md:1-27` |

...
| 编号 | 功能名 | 描述 | 一句话收口目标 |
|------|--------|------|----------------|
| F1 | Shared Build Freeze | 冻结 `jwt-shared` 的独立构建 / 测试 / 发布前提 | ✅ `jwt-shared` 不再成为 auth 链路的隐性 blocker |
| F2 | Endpoint Test Baseline | 为 ZX5 / RH public endpoints 建立统一直达测试纪律 | ✅ `后续 phase 的 public endpoint 都有统一验收底座` |
| F3 | Storage Bootstrap Prep | 把 KV/R2/binding/tooling readiness 显式化 | ✅ `RH4/RH5 不再依赖“应该已经有资源”` |

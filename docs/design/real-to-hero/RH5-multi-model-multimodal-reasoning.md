...
| 术语 | 定义 | 备注 |
|------|------|------|
| `model_id`（public surface）/ `model`（canonical） | 客户端 schema 字段名为 `model_id`（在 `SessionStartBodySchema` / `SessionMessagesBodySchema` 内）；canonical runtime 字段为 `model`（`CanonicalLLMRequest.model`）；ingress 适配层负责映射 | first-wave 可选但不能被 silent drop |
| multimodal | message content 含 `image_url` part | 依赖 RH4 file pipeline；`/messages` ingress 当前 `kind` 仅接受 `text|artifact_ref`，需扩 `image_url` |
| reasoning effort | `low|medium|high` 推理强度提示；通过 `CanonicalLLMRequest.reasoning?.effort` 承载 | 不支持的模型必须显式拒绝；`reasoning` 字段当前在 canonical 中**不存在**，RH5 必须新增 |
| model registry | D1 `nano_models` + runtime capability 对齐 | `/models` 是对外视图；`ModelCapabilities` 当前无 `supportsReasoning`，需扩 |
| context window | `ModelCapabilities.contextWindow`（camelCase）| 当前两个模型均为 128_000，charter §7.6 要求修正为 131_072（131K）|
...
- **[S0]** **schema 前置扩展**（必须先于 S1-S4）：
  - `packages/nacp-session/src/messages.ts` `SessionStartBodySchema` / `SessionMessagesBodySchema` 新增 `model_id?: string` + `reasoning?: { effort: "low"|"medium"|"high" }`
  - `workers/agent-core/src/llm/canonical.ts` `CanonicalLLMRequest` 新增 `reasoning?: { effort: "low"|"medium"|"high" }` 字段
  - `workers/agent-core/src/llm/registry/models.ts` `ModelCapabilities` 新增 `supportsReasoning: boolean` + 可选 `reasoningEfforts?: ("low"|"medium"|"high")[]`；`CapabilityName` 增加 `"reasoning"`
  - `workers/agent-core/src/llm/request-builder.ts` 增加 reasoning capability validation（不支持模型显式 throw `CAPABILITY_MISSING`，参考现有 vision check 模式）
  - `workers/agent-core/src/llm/adapters/workers-ai.ts` 增加 reasoning effort → Workers AI provider 参数翻译
- **[S1]** 13+4+8 `nano_models` registry 与 `/models` 一致；migration 011（如需）含 reasoning capability seed
- **[S2]** `model_id` 进入 start/messages surface 并一路透传到 canonical `model` 字段（schema 层 `model_id`，runtime 层 `model`，ingress 适配层完成映射）
- **[S3]** image_url / vision capability 真正进入执行路径：
  - **修正当前 `gateway.ts` 注册的 2 个模型 `supportsVision: false`**（`llama-4-scout` 实际支持 vision，应改为 `true`；或在 RH5 注册新 vision-capable 模型）
  - **扩 `/messages` ingress：`user-do.ts:1456` 的 `kind` 当前仅接受 `'text' | 'artifact_ref'`，必须新增 `'image_url'` 支持，并把 ingress shape 转为 `CanonicalLLMRequest.messages[].content[].ImageUrlContentPart`**
- **[S4]** reasoning effort 进入 canonical request 与 adapter 翻译（依赖 [S0]）
...
- **核心逻辑**：image 上传由 RH4 解决，RH5 负责让 image_url 不再被 silent-drop，而是进入 capability check 与 provider invocation。**真实状态澄清**：当前并非"silent-drop"——`request-builder.ts:81-92` 会**显式 throw `CAPABILITY_MISSING`**；但当前注册的 2 个模型均 `supportsVision: false`，且 `/messages` ingress `kind` 不接受 `image_url`，所以 image 输入根本进不到 capability check。**实施步骤**：(a) 修正 `llama-4-scout` 的 `supportsVision: true` 或注册新 vision 模型；(b) `user-do.ts:1456` 入口 `kind` 增加 `'image_url'`；(c) ingress 把 image_url 映射为 `CanonicalLLMRequest.messages[].content[].ImageUrlContentPart`；(d) 已有 vision capability check 直接复用。
- **边界情况**：
  - 非 vision 模型接到 image_url 必须显式报 capability missing（**已建成**，参见 §8.4）。
...
| 文件:行 | 内容 | 借鉴点 | 备注 |
|---------|------|--------|------|
| `packages/nacp-session/src/messages.ts:18-25` | `session.start` 目前只有 `cwd/initial_context/initial_input` | RH5 需把 `model_id` + `reasoning` 带入 public schema | current schema gap |
...
| `workers/orchestrator-core/src/user-do.ts:1456` | `/messages` ingress `kind` 当前仅 `'text' | 'artifact_ref'` | RH5 必须新增 `'image_url'` 支持 | current ingress block-out |
| `workers/agent-core/src/llm/canonical.ts:67-77` | `CanonicalLLMRequest` 当前**无** `reasoning` 字段 | RH5 [S0] 的明确扩展点 | current canonical gap |
| `workers/agent-core/src/llm/canonical.ts:46-50` | `ImageUrlContentPart` 已存在于 canonical | RH5 vision 路径直接复用 | already built |

### 8.4 已建成但未激活的资产（RH5 直接复用）

| 文件:行 | 内容 | RH5 中的角色 |
|---------|------|---------------|
| `workers/agent-core/src/llm/request-builder.ts:81-92` | 完整 vision capability check + `CAPABILITY_MISSING` throw | F3 不需要重新写 capability error 模式，沿用即可 |
| `workers/agent-core/src/llm/canonical.ts:46-50` | `ImageUrlContentPart` canonical 类型已存在 | F3 ingress 映射的目标 shape |
...
|------|--------|-------------|------------|
| GPT-R5 / kimi-R6 / deepseek-H5-5 共识 | 三方 | reasoning capability law 未落到现有 registry/schema 结构（CanonicalLLMRequest 无 reasoning；ModelCapabilities 无 supportsReasoning；CapabilityName 缺 reasoning；request-builder 缺 reasoning validation；adapter 缺 reasoning translation）| 新增 §5.1 [S0] 一次性列出全部 schema/registry/builder/adapter 改造点 |
| GLM-R4 / kimi-R6 共识 | GLM/kimi | `model_id` vs `model` 字段名漂移；`context_window` vs `contextWindow` 漂移 | §1.1 关键术语明确 public surface=`model_id`、canonical=`model`、ingress 适配层映射；context window 字段名修正为 camelCase |
| GLM-R16 | GLM | 当前 2 个模型均 `supportsVision: false`，vision 激活不是"切 flag"那么简单 | §5.1 [S3] / §7.2 F3 改写实施步骤（修正 supportsVision 标记或注册新模型 + ingress 适配 image_url） |
| GLM-R9 | GLM | `/messages` ingress `kind` 当前仅 `text | artifact_ref`，image_url 会被 400 拒绝 | §5.1 [S3] 步骤 (b)/(c)；§8.3 加 `user-do.ts:1456` 引用 |
| deepseek-H5-1 | deepseek | 当前 contextWindow 128_000 与 charter §7.6 要求 131K 不一致 | §1.1 加注；§5.1 [S1] 含 model registry 修正 |
| deepseek-H5-4 | deepseek | vision 基础设施已建成（`ImageUrlContentPart` + request-builder vision check）| §8.3 / §8.4 显式列出，避免 action-plan 重复实装 |

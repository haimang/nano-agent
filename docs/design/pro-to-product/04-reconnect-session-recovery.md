...
- `docs/design/pro-to-product/00-agent-loop-truth-model.md` — T3/T4 truth gates 与 cannot-close discipline。
- `docs/design/pro-to-product/01-frontend-trust-contract.md` — recovery bundle 的前端最小状态输入。
...
- **D1 纪律**：PP3 默认 zero migration；若 replay/detached 语义最终需要 schema 例外，只能按 charter §4.5 走 `018+` 受控变更。
...
- **[S5] T4 session state truth ownership** — PP3 closure 必须给出“phase / active-turn / pending 等价状态可恢复”的直接证据，PP6 只负责把该 truth 写进 docs。
...
> **PP3 启动 gate**：只有当 PP1 closure 文档已经显式声明 `session-do-runtime.ts` 的 ask/elicitation wakeup 主线已稳定，且不存在仍在进行中的改动会继续重写该文件的 live interrupt 语义时，PP3 才能进入主实现。
...
| 编号 | 功能名 | 描述 | 一句话收口目标 |
|------|--------|------|----------------|
| F1 | Replay Persistence + Cursor Law | `last_seen_seq` 与 replay persistence 的使用规则 | ✅ reconnect 不丢已知新帧 |
...
| F5 | Session State Truth | phase / active-turn / pending 等价状态可恢复 | ✅ T4 有单一 owner |

### 7.2 详细阐述

#### F1: Replay Persistence + Cursor Law

- **输入**：WS query `last_seen_seq`、session `relay_cursor`、agent stream frames、persisted replay fragment。
- **输出**：从 replay cursor 之后转发 event frames。
- **主要调用者**：User DO WS runtime。
- **核心逻辑**：`ws-runtime.ts:133-145` 当前用 `clientLastSeenSeq === null ? entry.relay_cursor : Math.min(entry.relay_cursor, clientLastSeenSeq)` 计算 replay cursor，并转发 `seq > cursor`；同时 `session-do-persistence.ts` 侧必须实现 replay fragment 的持久化与恢复对称，否则只修 attach/restore 任一端都不足以形成真实 replay closure。
...
- **核心逻辑**：当前 `surface-runtime.ts:280-319` 的 HTTP resume 已能在 `last_seen_seq > relay_cursor` 时返回 `replay_lost` 并记录 `session.replay_lost`；PP3 必须继续明确 WS attach 的等价 degraded 路径，或在 docs 中冻结“先 HTTP resume 再 WS attach”的恢复顺序，而不能让 WS gap 保持 silent。
...
#### F5: Session State Truth

- **输入**：phase、active turn、pending interaction、runtime version、recovery bundle。
- **输出**：T4 所需的前端可恢复 session state。
- **主要调用者**：frontend、PP3 closure、PP6 docs。
- **核心逻辑**：PP3 是 T4 的运行时证据 owner；PP6 只负责把该 truth 回填为 public contract，不替代运行时证据。
- **边界情况**：如果只能恢复 latest state 而无法恢复 pending/active truth，PP3 不得宣称 T4 已闭合。
- **一句话收口目标**：✅ **T4 不再分散在多个 phase 之间无人负责。**
...
| `workers/agent-core/src/host/do/session-do-persistence.ts:154-176,193-222` | helper checkpoint writes exist, but `replayFragment` still hard-coded `null` and restore only restores main checkpoint | PP3 必须同时修 persist + restore 两端 |
...
|----------------|------|----------|----------|------|----------|
| D-04-1 | PP3 是否承诺 exactly-once replay？ | PP3/PP6 | 否 | proposed | 本设计 + `docs/design/pro-to-product/PPX-qna.md` Q12 |
| D-04-2 | 是否支持多活动 attachment？ | PP3 | 否，single attachment | frozen/current code | `ws-runtime.ts` + `docs/design/pro-to-product/PPX-qna.md` Q13 |
| D-04-3 | replay gap 是否允许 silent latest-state fallback？ | PP3 | 否，必须 degraded | proposed | 本设计 + `docs/design/pro-to-product/PPX-qna.md` Q14 |
...
5. T4 session state truth 的运行时证据 owner 明确归属 PP3。

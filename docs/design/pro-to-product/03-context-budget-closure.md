...
| 项目 | 判定 | 理由 | 后续落点 |
|------|------|------|----------|
| `context_compact` confirmation | defer/optional | enum 已有，caller 未 live | PP2 可登记不强制 |
...
3. **取舍 3**：我们选择 **honest no-op 标注** 而不是 **把 `{tokensFreed:0}` 当成功**
   - **为什么**：当前 `requestCompact()` no-op 会误导前端。
   - **我们接受的代价**：PP2 可能先暴露 degraded/not-wired。
   - **未来重评条件**：agent-core compact 真接线后取消 degraded。
...
- **主要调用者**：agent-core。
- **核心逻辑**：当前 `runtime-mainline.ts:833-836` 返回 `{ tokensFreed: 0 }`，必须替换为 context-core durable compact 或 explicit degraded。
...
- **性能目标**：probe/preview ≤3s alert threshold；runtime preflight 不应阻塞正常短会话。
...
|----------------|------|----------|----------|------|----------|
| D-03-1 | 是否新增 compact jobs 表？ | PP2 | 否，复用 checkpoint | frozen | HPX-O2 / current docs |
| D-03-2 | auto compact 未接线时是否可写 live？ | PP2/PP6 | 否 | proposed | 本设计 |
| D-03-3 | PP2 是否必须引入 LLM summary？ | PP2 | 否，先 truth/boundary | proposed | 本设计 |

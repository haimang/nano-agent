...
| D-06-2 | stream retry 是否必须与 non-stream 对齐？ | PP5 | PP5 closure 前必须冻结为「内部 retry」或「显式 degraded + client retry + docs truth」之一,不允许保留两条 open option(与 PPX-qna Q19 reasoning 同步) | proposed | `docs/design/pro-to-product/PPX-qna.md` Q19 |

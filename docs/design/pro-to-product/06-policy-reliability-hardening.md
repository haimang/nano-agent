...
- `docs/design/pro-to-product/00-agent-loop-truth-model.md` — T6 Policy / reliability truth gate 与 cannot-close discipline。
- `docs/design/pro-to-product/01-frontend-trust-contract.md` — runtime / error / degraded 在前端合同中的边界。
...
- **边界情况**：db missing 当前返回 `ask/unavailable`，随后被 runtime 翻译成 `tool-permission-required` error；根据 `PPX-qna Q20` 的 owner 口径，PP5 必须把这类 unavailable 改成独立的 fail-visible unavailable/degraded 路径，优先避免继续复用 ask 语义。
...
- **核心逻辑**：non-stream executor 有 retry/backoff/429 key rotation；stream executor 目前失败直接 throw。`PPX-qna Q19` 已冻结 first-wave 路径为 **显式 degraded + client retry + docs truth**，而不是在 PP5 内追求与 non-stream 的内部 retry 完全对齐。
...
|----------------|------|----------|----------|------|----------|
| D-06-1 | config-only 字段是否保留？ | PP5/PP6 | 保留但标明 not-enforced | frozen | `docs/design/pro-to-product/PPX-qna.md` Q18 |
| D-06-2 | stream retry 是否必须与 non-stream 对齐？ | PP5 | 否；first-wave 冻结为「显式 degraded + client retry + docs truth」 | frozen | `docs/design/pro-to-product/PPX-qna.md` Q19 |
| D-06-3 | policy unavailable 是否允许 fallback allow？ | PP5 | 不允许 silent allow；应改成独立 unavailable/degraded 的 fail-visible 路径 | frozen | `docs/design/pro-to-product/PPX-qna.md` Q20 |

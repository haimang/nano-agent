...
- **核心逻辑**：GET/PATCH/ETag 证明字段可配置；必须进一步证明执行层读取该字段。
...
- **边界情况**：db missing 当前返回 `ask/unavailable`，前端/agent 不应误解为正常 user ask。
...
- **需要进入 QNA register 的问题**：
  - D-06-2 可在 action-plan 中决定“实现 stream retry”还是“明确 degraded + client retry”。

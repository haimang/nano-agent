...
|--------|------------------|----------|-----------------------------|
| 新增 confirmation kind | tool cancel / custom ask 诱因 | HP5 Q18(`docs/issue/hero-to-pro/HP5-closure.md` §5)已冻结 7-kind;PPX-qna Q8 仅复用此冻结 | 新 charter 才能改 |
...
- **D1 纪律**：PP1 默认 zero migration；若后续实现声称需要 confirmation schema 例外，只能按 charter §4.5 申请 `018+` 编号，不能在 action-plan 内默认追加。
...
| `tool.call.cancelled` | out-of-scope | HP5 Q18 禁止加入 confirmation(注意:此处的 Q18 指 HP5-closure §5,与 PPX-qna Q18 无关) | workbench/tool-call docs |
...
| `workers/agent-core/src/host/do/session-do-runtime.ts:378-415` | permission + elicitation 共用 row-first await substrate | 可复用 wait primitive |
...
|----------------|------|----------|----------|------|----------|
| D-02-1 | `ask` 是否允许继续返回 tool error？ | PP1 | 否，必须 interrupt | frozen | `docs/design/pro-to-product/PPX-qna.md` Q6 |
| D-02-2 | confirmation decision 是否改成 WS 输入？ | PP1/PP6 | 否，维持 HTTP | frozen | `type-direction-matrix.ts` |
| D-02-3 | 是否扩展 confirmation kind？ | PP1 | 否 | frozen | HP5-closure §5 (HP5 Q18) + `docs/design/pro-to-product/PPX-qna.md` Q8 |
...
5. PP1 closure 文档必须显式说明 `session-do-runtime.ts` 的 ask/elicitation wakeup 主线已稳定，便于 PP3 判断共享 owner file 是否可进入下一阶段。
...
- **需要进入 QNA register 的问题**：
  - 无新增;PP1 主要 owner decisions(D-02-1 / D-02-2 / D-02-3)已集中到 `docs/design/pro-to-product/PPX-qna.md` Q6 / Q7 / Q8,PP1 action-plan 与 closure 只消费该答案。

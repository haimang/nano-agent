# RFC: `workspace.fs.*` Remote RPC（方向性草案）

> 状态：Directional RFC / pre-worker-matrix 输入  
> 当前阶段：**只冻结方向，不冻结 wire 实装**  
...
> 说明：以下是 **推荐命名**，用于 worker-matrix P0 继续细化；本阶段不要求在 `@nano-agent/nacp-core` 立即 ship。
...
## 7. 进入 worker-matrix P0 前的最低对齐要求
...
这份 RFC 的价值不在于“现在就做远程文件系统”，而在于**先把 filesystem-core 的未来 service-binding seam 限制在一条很窄、很诚实的 typed path 上**。这样 W4/W5 之后进入 worker-matrix 时，就不会一边复用 `WorkspaceNamespace`，一边又在跨 worker 边界重新发明 shell-shaped file protocol。

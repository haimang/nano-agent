...
## Running tests(RH0 P0-A2 落地)

仓库测试矩阵分布在 6 个 worker + 6 个 package + root contracts。常用入口:

```bash
# Root pnpm test alias — pnpm-recursive 跑所有 workspace 包的 `test` script
NODE_AUTH_TOKEN=$NODE_AUTH_TOKEN pnpm test

# Per-worker(以 orchestrator-core 为例,其他 worker 同形)
NODE_AUTH_TOKEN=$NODE_AUTH_TOKEN pnpm --filter @haimang/orchestrator-core-worker test

# jwt-shared (单个 package)
NODE_AUTH_TOKEN=$NODE_AUTH_TOKEN pnpm --filter @haimang/jwt-shared test

# Root guardian / cross-e2e
NODE_AUTH_TOKEN=$NODE_AUTH_TOKEN pnpm test:contracts
NODE_AUTH_TOKEN=$NODE_AUTH_TOKEN pnpm test:cross-e2e

# Madge cycle baseline(RH6 enforce 0 cycle;RH0 后 baseline 10 个 carry-over)
pnpm check:cycles
```

NODE_AUTH_TOKEN 是 GitHub Packages classic PAT(scope `read:packages`),用于拉取
`@haimang/*` 内部 package。详见仓库根 `.npmrc` 顶部说明。

每个 worker 的 test 子目录(`workers/{name}/test/README.md` 当其存在时)记录测试
文件的覆盖边界与 carry-over。

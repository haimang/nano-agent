...
>
> 本文件覆盖 session **生命周期与 transport** 主线（start / input / cancel / close / delete / title / messages / status / timeline / history / verify / resume / conversation detail）。
...
| 404 | `session-not-found` | UUID 未在 `nano_conversation_sessions` |
...
| 404 | `session-not-found` / `conversation-not-found` | UUID 不存在 |

...
1. Pack `@haimang/nacp-core` and `@haimang/nacp-session`
2. Install those tarballs into a throwaway copy of this package
3. If you use pnpm, add an override for transitive `@haimang/nacp-core` so the packed `nacp-session` dependency does not fall back to the public registry

...
Lifecycle-governance hooks for nano-agent: the 8-event catalog, outcome
aggregation, single-entry dispatcher, local + service-binding runtimes,
NACP-Core/Session codec, audit-record builder, and session snapshot/restore.
...
execute sequentially by source priority, and short-circuit as soon as a
handler returns `block` or (where allowed) `stop`. **Non-blocking**
